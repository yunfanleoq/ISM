---
name: 大屏假死数据修复
overview: 按 2026-07-21 现场问题会「一锅端」保障可用：假死转圈、假数据、RealDataChanel 丢包、品牌/状态中文化、drop 可诊断日志。当晚出方案/热修包，次日现场验证。
todos:
  - id: p0-break-event-loop
    content: 打断 ViewRealTable/ISMRender 的 NavDatapointNeedHydrate ↔ PageUpdate 环路（in-flight/指纹短路/Loading 必释放）
    status: completed
  - id: p0-remove-chart-mock
    content: 停用 buildDimensionSample；给中航信趋势图挂 energyOverviewRole；无数据只显示暂无历史数据
    status: completed
  - id: p0-ws-drop-rootcause
    content: 定位 RealDataChanel 丢包根因（断网重连必现/20 万点）；合并推送或降频；增强 drop 诊断日志写文件
    status: completed
  - id: p0-branding-status-zh
    content: 用 docx 已提取素材替换登录页/首页 Logo 与文案（循安）；设备状态英文改中文
    status: completed
  - id: p1-fetchall-and-verify
    content: 去掉 fetchDeviceDatapoints 的 fetchAll；diagnose 脚本核对分页二进制；次日邹承举验收
    status: completed
  - id: p1-patch-docs
    content: 打现场补丁 + 同步 docs/ 与 .cursor/plans/；保留排查期详细日志开关
    status: completed
isProject: false
---
