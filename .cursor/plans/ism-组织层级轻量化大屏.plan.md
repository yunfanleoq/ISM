---
name: 组织层级大屏
overview: 将大屏改为以组织层级导航、设备摘要与按需测点分页为核心的轻量运行态界面，并建立明确的组件、接口和数据载荷预算，避免预生成大量页面或一次性加载海量点位。
todos:
  - id: define-performance-budgets
    content: 定义首屏、组织页、设备页的组件数、接口数、数据量和交互延迟预算
    status: pending
  - id: refactor-page-information-architecture
    content: 重构组织总览、组织下钻和设备详情三层页面职责与导航协议
    status: pending
  - id: add-demand-data-api
    content: 实现设备测点服务端筛选分页、摘要数据和按需历史查询接口
    status: pending
  - id: lazy-load-rendering
    content: 实现树节点、设备卡片、趋势图和测点组件的按需加载与虚拟化
    status: pending
  - id: migrate-dashboard-config
    content: 迁移当前大屏配置为轻量模板，移除预生成的冗余设备页面
    status: pending
  - id: validate-performance
    content: 完成接口、浏览器性能、分页稳定性和数据一致性验收
    status: pending
isProject: false
---
