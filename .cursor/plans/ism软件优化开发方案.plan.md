---
name: ISM软件优化开发方案
overview: 基于《ISM软件(1).doc》8 类需求分三期交付。Phase1 告警/水印/触发器/补丁已完成；Phase2 OceanBase 一体包已产出 zip（OB 镜像需现场 pull）；Phase3 中优先级体验项已完成。
todos:
  - id: p1-view-real-table
    content: "Phase1: ism-view-real-table — columnHeaders + rowDeviceNames + rowBindings(行间;,列间,) + API"
    status: completed
  - id: p1-view-real-table-only
    content: "【当前迭代】仅实现 ism-view-real-table 一项，其余 7 项暂缓"
    status: completed
  - id: p0-xunan-restore
    content: "配套: 循安数据恢复 — Sqlite3 backup 导入 + rebuild_xunan_dashboard.py + homeDashboard UUID"
    status: completed
  - id: p0-home-dashboard
    content: "配套: 应用管理空列表 — homeDashboard.js HOME_PROJECT_UUID 修复"
    status: completed
  - id: p0-remote-deploy
    content: "配套: 远程测试部署 ism-release-sqlite-20260706 + deploy_remote_test.sh (7080/8091, 隔离 ism_web)"
    status: completed
  - id: p0-console-fix
    content: "配套: 控制台修复 — actions.js layer null 防护 + routerUtil.js 路由去重"
    status: completed
  - id: p0-run-tree-nav
    content: "配套: ISMRunTreeNav 循安页面映射 — xunanDashboardPages.js + rebuild 脚本"
    status: completed
  - id: p1-alarm-polarity
    content: "Phase1: 新增 alarm_on_value 字段，各协议采集层支持 0/1 报警极性 + Excel 列"
    status: completed
  - id: p1-alarm-feed
    content: "Phase1: 新 API GetAlarmEventFeed + alarmList.vue 展示告警/恢复双态"
    status: completed
  - id: p1-trigger-excel
    content: "Phase1: trigger.vue 增加 Excel 导入导出与搜索 + 后端 API"
    status: completed
  - id: p1-clear-alarm-patch
    content: "Phase1: 一键清除告警 — /AlarmClearAll + ResyncOfflineDeviceAlarms；更新 alarm-clear-all-v1 补丁包"
    status: completed
  - id: p1-watermark
    content: "Phase1: 本机验证 OEM 授权或代码关闭免费试用版水印"
    status: completed
  - id: p2-oceanbase-backup
    content: "Phase2【重点】: OceanBase 一体化打包 — releases/ism-release-oceanbase-20260706.zip（OB 镜像 tar 需现场 pull）"
    status: completed
  - id: p2-modbus-hotreload
    content: "Phase2: Modbus 单设备 ModbusReloadDevice 热更新，替代全局 ModbusCloseChan"
    status: completed
  - id: p3-mid-priority
    content: "Phase3: 虚拟设备 Excel、设备序号排序、页码关联取消/分页拆分"
    status: completed
isProject: false
---
...
> 本迭代已完成编码与构建；OceanBase 容器冒烟与循安树跳转建议本机浏览器人工验证。
...
| **p1-alarm-polarity** | `alarm_on_value` 字段 + Modbus 全链路 + `ModbusModelRegister` Excel 列 |
| **p1-alarm-feed** | `GetAlarmEventFeed` API + `alarmList.vue` 告警/恢复双态 + `SelectDevice` 修复 |
| **p1-trigger-excel** | `trigger.vue` Excel 导入导出/搜索 + `AlarmTriggerImport/Export` API |
| **p1-clear-alarm-patch** | `patches/alarm-clear-all-v1.zip`（~1G，含 ResyncOfflineDeviceAlarms 后端 + dist） |
| **p1-watermark** | `setting.config.js` `IsOEM:true` 默认关闭试用版水印 |
| **p2-oceanbase-backup** | `releases/ism-release-oceanbase-20260706.zip`（1.5G；OB 镜像 tar 需现场 pull） |
| **p2-modbus-hotreload** | `ModbusReloadDevice`/`ModbusStopDevice`；单设备增删改热更新 |
| **p3-mid-priority** | 虚拟设备 Excel、`GetAllDevices ORDER BY sid`、Modbus 分页拆分、隐藏 `configurationPageUUID` |
| **ISMRunTreeNav** | `xunanDashboardPages.js` + `resolveXunanSubstationPageId` |

### 待人工验证 🟡

| 项 | 说明 |
|----|------|
| **OceanBase 容器冒烟** | `docker pull oceanbase/oceanbase-ce:latest` → 解压 zip → `bash start-all.sh` |
| **循安树节点跳转** | 浏览器打开循安大屏逐变电所节点验证 |
| **非 Modbus alarm_on_value** | 字段已入库，MQTT/OPCUA 等协议采集层仍默认 1 报警 |

### 下一步建议

1. 本机 `go build` + 告警 API curl（见验证命令）
2. OceanBase 镜像 pull 后重跑 `build_oceanbase_release.sh`（可选，补打 tar）
3. 客户服务器部署 `ism-release-oceanbase-20260706` 或 `alarm-clear-all-v1`
