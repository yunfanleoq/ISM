...
<div class="email">
	...
	设备离线
	{{else if eq  .AlarmMessage "VideoManager.VideoOffline"}}
	视频设备离线
	{{else}}
	{{ .AlarmMessage }}
	{{end}}
	{{else}}
	{{if eq  .AlarmClearMessage "device.DeviceStatusOnline"}}
	设备在线
	{{else if eq  .AlarmClearMessage "VideoManager.VideoOnline"}}
	视频设备在线
	{{else}}
	{{ .AlarmClearMessage }}
	{{end}}
