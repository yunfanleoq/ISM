...
func NewClient(handler ClientHandler) Client {
	return &client{packager: handler, transporter: handler, sendPackerID: 0}
}
...
func (mb *client) readArea(area int, dbNumber int, start int, amount int, wordLen int, buffer []byte) (err error) {
	...
	for totElements > 0 && err == nil {
		...
		binary.BigEndian.PutUint16(requestData[11:], uint16(mb.sendPackerID))
		mb.sendPackerID++
		...
		if err == nil {
			...
			return fmt.Errorf(ErrorText(errTCPReceiveTimeout))
		}
...
func (mb *client) send(request *ProtocolDataUnit) (response *ProtocolDataUnit, err error) {
	...
	if err != nil {
		return nil, fmt.Errorf("s7: read data timeout")
	}
	...
	if err = mb.packager.Verify(request.Data, dataResponse); err != nil {
		return nil, fmt.Errorf("s7: read data Verify error")
	}
