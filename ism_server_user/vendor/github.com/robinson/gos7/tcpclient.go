...
func (mb *tcpTransporter) Send(request []byte) (response []byte, err error) {
	...
	// Send data
	// fmt.Println("s7: sending % x", request)
	...
	for !done && err == nil {
		...
		if _, err = mb.conn.Read(data[:4]); err != nil {
			fmt.Println("%T %v", err, err)
			return nil, err
		}
	...
	_, err = mb.conn.Read(data[7:length])
	if err != nil {
		return nil, err
	}
	response = data[0:length]
	// fmt.Println("s7: received % x\n", response)
	return response, nil
}
...
func (mb *tcpPackager) Verify(request []byte, response []byte) (err error) {
	var requestPackID uint16
	var responsePackID uint16

	if len(response) < 13 {
		return fmt.Errorf("response len error less 13 byte")
	}
	requestBytesBuffer := bytes.NewBuffer(request[11:])
	binary.Read(requestBytesBuffer, binary.BigEndian, &requestPackID)
	responseBytesBuffer := bytes.NewBuffer(response[11:])
	binary.Read(responseBytesBuffer, binary.BigEndian, &responsePackID)
	if requestPackID != responsePackID {
		return fmt.Errorf("packid error")
	}
	return nil
}
