...
func (s7 *Helper) GetStringAt(buffer []byte, pos int, maxlength int) string {
	l := uint8(buffer[pos+1])
	if pos+2+int(l) > maxlength || pos+2 > maxlength {
		return ""
	} else {
...
func (s7 *Helper) GetWStringAt(buffer []byte, pos int, maxLength int16) string {
	var l, max int16
	var i int
	var s string
	s7.GetValueAt(buffer, pos+0, &max)
	s7.GetValueAt(buffer, pos+2, &l)
	if max > maxLength || int16(l) > maxLength {
		return ""
	}
	bs := buffer[pos+4:]
	for i < int(l) {
