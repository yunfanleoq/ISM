...
func (asdu *ASDU) parseInformationElement(data []byte, ie *InformationElement) {
	...
	case CCsNa1:
		switch asdu.cot {
		case CotActCon:
			_lg.Debugf("receive i frame: confirmation of counter interrogation [时钟确认]")
			asdu.sendSFrame = true
		case CotActTerm:
			_lg.Debugf("receive i frame: termination of counter interrogation [时钟结束]")
			asdu.sendSFrame = true
		default:
			asdu.sendSFrame = true
		}
	...
	default:
		asdu.sendSFrame = true
		_lg.Debugf("unsupported type: TypeID[%X], COT[%X]", asdu.typeID, asdu.cot)
	}
}
