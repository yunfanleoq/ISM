...
func (d *Decoder) ContextPriorityArray(expectedTagNumber byte) []string {
	// PriorityArray return data Parse
	if d.err != nil {
		return nil
	}
	...
	resArray := make([]string, 0, 16)
	for i := 1; i <= 16; i++ {
		res := d.AppDataPriorityArray()
		if d.err != nil {
			return nil
		}
...
//AppDataPriorityArray PriorityArray return data Parse,return all string
func (d *Decoder) AppDataPriorityArray() string {
	if d.err != nil {
		return ""
	}
	_, tag, err := decodeTag(d.buf)
	...
	switch tag.ID {
	case applicationTagNull:
		return bacnet.PropertyNullValue
	case applicationTagUnsignedInt:
		val, err := decodeUnsignedWithLen(d.buf, int(tag.Value))
		if err != nil {
			d.err = fmt.Errorf("decodeAppData: read ObjectID: %w", err)
