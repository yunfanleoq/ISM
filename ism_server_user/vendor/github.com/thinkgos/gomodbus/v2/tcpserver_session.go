...
func (sf *ServerSession) running(ctx context.Context) {
	...
	for {
		...
		for rdCnt, length := 0, tcpHeaderMbapSize; rdCnt < length; {
			...
			if rdCnt >= tcpAduMaxSize || length > tcpAduMaxSize {
				err = fmt.Errorf("modbus tcp adu length %d over max %d", length, tcpAduMaxSize)
				return
			}
...
func (sf *ServerSession) frameHandler(requestAdu []byte) error {
	...
	if err != nil {
		funcCode |= 0x80
		rspPduData = []byte{ExceptionCodeIllegalDataValue}
	}
	...
	if funcCode == 1 || funcCode == 2 || funcCode == 3 || funcCode == 4 {
		binary.BigEndian.PutUint16(responseAdu[4:], uint16(3+len(rspPduData)))
	} else {
	...
	if funcCode == 1 || funcCode == 2 || funcCode == 3 || funcCode == 4 {
		responseAdu = append(responseAdu, byte(len(rspPduData)))
	}
