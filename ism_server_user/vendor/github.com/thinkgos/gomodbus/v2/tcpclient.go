...
func NewTCPClientProvider(prefix string, modbusType byte, address string, opts ...ClientProviderOption) *TCPClientProvider {
	p := &TCPClientProvider{
		address:       address,
...
// SendRawFrame send raw adu request frame
func (sf *TCPClientProvider) SendRTURawFrame(aduRequest []byte) (aduResponse []byte, err error) {
	sf.mu.Lock()
	defer sf.mu.Unlock()
	...
	for {
		...
		if cnt, err = sf.conn.Read(data[:]); err == nil {
			break
		}
		if sf.autoReconnect == 0 {
			return
...
func (sf *TCPClientProvider) SetConnect(conn net.Conn) error {
