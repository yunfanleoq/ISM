...
type Dlt645TCPClientProvider struct {
	logger
	address string
	serialPort
	protocolType byte
...
var _ ClientProvider = (*Dlt645TCPClientProvider)(nil)

// request pool, all TCP client use this pool

// NewTCPClientProvider allocates a new TCPClientProvider.
func NewDlt645TCPClientProvider(prefix string, modbusType byte, address string, opts ...ClientProviderOption) *Dlt645TCPClientProvider {
	p := &Dlt645TCPClientProvider{
		address:       address,
...
func (sf *Dlt645TCPClientProvider) setPrefixHandler(handler PrefixHandler) {
	sf.PrefixHandler = handler
}

// SendAndRead 发送数据并读取返回值
func (sf *Dlt645TCPClientProvider) SendAndRead(p *Protocol) (aduResponse []byte, err error) {
...
func (sf *Dlt645TCPClientProvider) Send(p *Protocol) (err error) {
	bf := bytes.NewBuffer(make([]byte, 0))
	err = sf.EncodePrefix(bf, sf.BeforeEnable)
	if err != nil {
		return err
...
// ReadRawFrame 读取返回数据
func (sf *Dlt645TCPClientProvider) ReadRawFrame() (aduResponse []byte, err error) {
...
func (sf *Dlt645TCPClientProvider) SendRawFrameAndRead(aduRequest []byte) (aduResponse []byte, err error) {
	sf.mu.Lock()
	defer sf.mu.Unlock()
...
func (sf *Dlt645TCPClientProvider) SendRawFrame(aduRequest []byte) (err error) {
	// if err = sf.connect(); err != nil {
	// 	return
	// }
	var timeout time.Time
...
func (sf *Dlt645TCPClientProvider) Connect() error {
	sf.mu.Lock()
	err := sf.connect()
	sf.mu.Unlock()
	return err
...
func (sf *Dlt645TCPClientProvider) IsConnected() bool {
	sf.mu.Lock()
	b := sf.isConnected()
	sf.mu.Unlock()
	return b
...
func (sf *Dlt645TCPClientProvider) SetAutoReconnect(cnt byte) {
	sf.mu.Lock()
...
func (sf *Dlt645TCPClientProvider) Close() (err error) {
	sf.mu.Lock()
	if sf.conn != nil {
		err = sf.conn.Close()
		sf.conn = nil
...
func (sf *Dlt645TCPClientProvider) SetSerialConfig(serial.Config) {}

func (sf *Dlt645TCPClientProvider) SetTCPTimeout(t time.Duration) {
	sf.timeout = t
}

// flush flushes pending data in the connection,
// returns io.EOF if connection is closed.
func (sf *Dlt645TCPClientProvider) flush(b []byte) (err error) {
...
func (sf *Dlt645TCPClientProvider) SetConnect(conn net.Conn) error {

	return nil
}
func (sf *Dlt645TCPClientProvider) SetBeforeCode(enable bool) error {
