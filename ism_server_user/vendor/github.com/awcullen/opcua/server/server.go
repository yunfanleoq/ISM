...
type Server struct {
	...
	allowedSecurityPolicyURIs            map[string]struct{}
	allowedMessageSecurityModes          map[ua.MessageSecurityMode]struct{}
...
func (srv *Server) buildEndpointDescriptions() []ua.EndpointDescription {
	...
	if srv.allowSecurityPolicyNone && srv.isSecurityPolicyAllowed(ua.SecurityPolicyURINone) && srv.isMessageSecurityModeAllowed(ua.MessageSecurityModeNone) {
	...
	for _, uri := range uris {
		if !srv.isSecurityPolicyAllowed(uri) || !srv.isMessageSecurityModeAllowed(ua.MessageSecurityModeSign) {
			continue
		}
		toks := []ua.UserTokenPolicy{}
		if srv.anonymousIdentityAuthenticator != nil {
	...
	for _, uri := range uris {
		if !srv.isSecurityPolicyAllowed(uri) || !srv.isMessageSecurityModeAllowed(ua.MessageSecurityModeSignAndEncrypt) {
			continue
		}
		toks := []ua.UserTokenPolicy{}
		if srv.anonymousIdentityAuthenticator != nil {
...
func (srv *Server) isSecurityPolicyAllowed(policyURI string) bool {
	// 未配置白名单时保持原行为，允许所有内置安全策略。
	if len(srv.allowedSecurityPolicyURIs) == 0 {
		return true
	}
	_, ok := srv.allowedSecurityPolicyURIs[policyURI]
	return ok
}

func (srv *Server) isMessageSecurityModeAllowed(mode ua.MessageSecurityMode) bool {
	// 未配置白名单时保持原行为，允许所有内置消息安全模式。
	if len(srv.allowedMessageSecurityModes) == 0 {
		return true
	}
	_, ok := srv.allowedMessageSecurityModes[mode]
	return ok
}
