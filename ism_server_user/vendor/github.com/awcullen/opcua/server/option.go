...
// WithSecurityPolicies limits the server endpoints to the given security policy URIs.
// If no URIs are provided, all built-in policies remain available.
func WithSecurityPolicies(policyURIs ...string) Option {
	return func(srv *Server) error {
		if len(policyURIs) == 0 {
			srv.allowedSecurityPolicyURIs = nil
			return nil
		}
		srv.allowedSecurityPolicyURIs = make(map[string]struct{}, len(policyURIs))
		for _, uri := range policyURIs {
			srv.allowedSecurityPolicyURIs[uri] = struct{}{}
		}
		return nil
	}
}

// WithMessageSecurityModes limits the server endpoints to the given message security modes.
// If no modes are provided, all built-in modes remain available.
func WithMessageSecurityModes(modes ...ua.MessageSecurityMode) Option {
	return func(srv *Server) error {
		if len(modes) == 0 {
			srv.allowedMessageSecurityModes = nil
			return nil
		}
		srv.allowedMessageSecurityModes = make(map[ua.MessageSecurityMode]struct{}, len(modes))
		for _, mode := range modes {
			srv.allowedMessageSecurityModes[mode] = struct{}{}
		}
		return nil
	}
}
