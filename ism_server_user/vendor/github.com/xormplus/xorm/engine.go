...
func (engine *Engine) DumpAllToFile(fp string, tables []string, tp ...schemas.DBType) error {
	f, err := os.Create(fp)
	if err != nil {
		return err
	}
	defer f.Close()
	return engine.DumpAll(f, tables, tp...)
}

// DumpAll dump database all table structs and data to w
func (engine *Engine) DumpAll(w io.Writer, gettables []string, tp ...schemas.DBType) error {
	tables, err := engine.DBMetas()
	if err != nil {
		return err
	}
	return engine.DumpTables(tables, gettables, w, tp...)
}
...
func (engine *Engine) DumpTables(tables []*schemas.Table, getTables []string, w io.Writer, tp ...schemas.DBType) error {
	return engine.dumpTables(tables, getTables, w, tp...)
}
func isText(Name string) bool {

	return strings.Contains(strings.ToLower(Name), strings.ToLower("Enum")) ||
		strings.Contains(strings.ToLower(Name), strings.ToLower("Set")) ||
		strings.Contains(strings.ToLower(Name), strings.ToLower("Json")) ||
		strings.Contains(strings.ToLower(Name), strings.ToLower("Jsonb")) ||
		strings.Contains(strings.ToLower(Name), strings.ToLower("Char")) ||
		strings.Contains(strings.ToLower(Name), strings.ToLower("NChar")) ||
		strings.Contains(strings.ToLower(Name), strings.ToLower("Varchar")) ||
		strings.Contains(strings.ToLower(Name), strings.ToLower("NVarchar")) ||
		strings.Contains(strings.ToLower(Name), strings.ToLower("TinyText")) ||
		strings.Contains(strings.ToLower(Name), strings.ToLower("Text")) ||
		strings.Contains(strings.ToLower(Name), strings.ToLower("NText")) ||
		strings.Contains(strings.ToLower(Name), strings.ToLower("MediumText")) ||
		strings.Contains(strings.ToLower(Name), strings.ToLower("LongText")) ||
		strings.Contains(strings.ToLower(Name), strings.ToLower("Uuid")) ||
		strings.Contains(strings.ToLower(Name), strings.ToLower("Clob")) ||
		strings.Contains(strings.ToLower(Name), strings.ToLower("SysName"))
}
func isTime(Name string) bool {

	return strings.Contains(strings.ToLower(Name), strings.ToLower("Date")) ||
		strings.Contains(strings.ToLower(Name), strings.ToLower("DateTime")) ||
		strings.Contains(strings.ToLower(Name), strings.ToLower("Time")) ||
		strings.Contains(strings.ToLower(Name), strings.ToLower("TimeStamp")) ||
		strings.Contains(strings.ToLower(Name), strings.ToLower("TimeStampz")) ||
		strings.Contains(strings.ToLower(Name), strings.ToLower("SmallDateTime")) ||
		strings.Contains(strings.ToLower(Name), strings.ToLower("Year"))
}

func isBlob(Name string) bool {

	return strings.Contains(strings.ToLower(Name), strings.ToLower("Binary")) ||
		strings.Contains(strings.ToLower(Name), strings.ToLower("VarBinary")) ||
		strings.Contains(strings.ToLower(Name), strings.ToLower("TinyBlob")) ||
		strings.Contains(strings.ToLower(Name), strings.ToLower("Blob")) ||
		strings.Contains(strings.ToLower(Name), strings.ToLower("MediumBlob")) ||
		strings.Contains(strings.ToLower(Name), strings.ToLower("LongBlob")) ||
		strings.Contains(strings.ToLower(Name), strings.ToLower("UniqueIdentifier")) ||
		strings.Contains(strings.ToLower(Name), strings.ToLower("Bytea"))
}

func isColNumeric(Name string) bool {

	return strings.Contains(strings.ToLower(Name), strings.ToLower("Bool")) ||
		strings.Contains(strings.ToLower(Name), strings.ToLower("Serial")) ||
		strings.Contains(strings.ToLower(Name), strings.ToLower("BigSerial")) ||
		strings.Contains(strings.ToLower(Name), strings.ToLower("Decimal")) ||
		strings.Contains(strings.ToLower(Name), strings.ToLower("Numeric")) ||
		strings.Contains(strings.ToLower(Name), strings.ToLower("Real")) ||
		strings.Contains(strings.ToLower(Name), strings.ToLower("INT")) ||
		strings.Contains(strings.ToLower(Name), strings.ToLower("Float")) ||
		strings.Contains(strings.ToLower(Name), strings.ToLower("Double")) ||
		strings.Contains(strings.ToLower(Name), strings.ToLower("Money")) ||
		strings.Contains(strings.ToLower(Name), strings.ToLower("BIGINT")) ||
		strings.Contains(strings.ToLower(Name), strings.ToLower("SmallMoney"))
}
func formatColumnValue(dstDialect dialects.Dialect, d interface{}, col *schemas.Column) string {
	...
	if isText(col.SQLType.Name) {
		var v string = ""

		v = fmt.Sprintf("%s", d)

		return "'" + strings.Replace(v, "'", "''", -1) + "'"
	} else if isTime(col.SQLType.Name) {
		var v = fmt.Sprintf("%s", d)
		timeStr := strings.Split(v, " ")
		var timeV = fmt.Sprintf("%s %s", timeStr[0], timeStr[1])
		t, _ := time.ParseInLocation("2006-01-02 15:04:05", timeV, time.Local)

		datetime := t.Format("2006-01-02 15:04:05")
		return fmt.Sprintf("'%s'", datetime)
	} else if isBlob(col.SQLType.Name) {
		if reflect.TypeOf(d).Kind() == reflect.Slice {
			return fmt.Sprintf("%s", dstDialect.FormatBytes(d.([]byte)))
		} else if reflect.TypeOf(d).Kind() == reflect.String {
			return fmt.Sprintf("'%s'", d.(string))
		}
	} else if isColNumeric(col.SQLType.Name) {
...
func findTablesName(tableName string, getTables []string) bool {
	for _, v := range getTables {
		if v == tableName {
			return true
		}
	}
...
func (engine *Engine) dumpTables(tables []*schemas.Table, getTables []string, w io.Writer, tp ...schemas.DBType) error {
	...
	_, err := io.WriteString(w, fmt.Sprintf("/*Generated by ISM %s, from %s to %s*/\r\n",
	...
	for i, table := range tables {
		...
		if !findTablesName(tableName, getTables) {
			continue
		}
		...
		DropTableSQL, _ := dstDialect.DropTableSQL(tableName)
		_, err = io.WriteString(w, DropTableSQL+";\n")
		if err != nil {
			return err
		}
		...
		// 	_, err = io.WriteString(w, dstDialect.DropIndexSQL(table.Name, index)+";\n")
		// 	if err != nil {
		// 		return err
		// 	}
		// }
		...
		var rows *core.Rows
		var err error
		if engine.ProjectUUid == "" {
			rows, err = engine.DB().QueryContext(engine.defaultContext, "SELECT "+colNames+" FROM "+engine.Quote(originalTableName))
		} else {
			rows, err = engine.DB().QueryContext(engine.defaultContext, "SELECT "+colNames+" FROM "+engine.Quote(originalTableName)+" where project_uuid='"+engine.ProjectUUid+"'")
		}
