...
func (session *Session) Import(r io.Reader) ([]sql.Result, error) {
	...
	for scanner.Scan() {
		...
		if strings.Contains(query, "sqlite3 to sqlite3") {
			backUpType = 1
			break
		} else if strings.Contains(query, "mysql to mysql") {
			backUpType = 2
			break
		}
	}
		...
		if driverName == "sqlite3" {
			if backUpType == 2 {
				...
				if strings.Contains(query, "CREATE TABLE IF NOT EXISTS") {
					query = strings.Replace(query, "AUTO_INCREMENT", "AUTOINCREMENT", -1)
					query = strings.Replace(query, "BIGINT(20)", "integer", -1)
					query = strings.Replace(query, "`id` INT(11)", "`id` integer", -1)
					query = strings.Replace(query, "DATETIME(3)", "DATETIME", -1)
					query = strings.Replace(query, "INT(11)", "integer", -1)
					query = strings.Replace(query, "ENGINE=InnoDB DEFAULT CHARSET utf8", "", -1)
					query = strings.Replace(query, "INT", "integer", -1)
					query = strings.Replace(query, "BIGINT", "integer", -1)
				}

				if strings.Contains(query, "CREATE TABLE IF NOT EXISTS `display_model_layer`") {
					query = "CREATE TABLE IF NOT EXISTS `display_model_layer` (`id` integer PRIMARY KEY AUTOINCREMENT NOT NULL, `created_at` DATETIME NULL, `updated_at` DATETIME NULL, `deleted_at` DATETIME NULL, `model_id` varchar(250) NULL, `page_name` varchar(250) NULL, `page_id` varchar(250) NULL, `is_home` integer NOT NULL, `page_type` integer NOT NULL,`is_login` integer NOT NULL, `layer` text NOT NULL, `components` TEXT NOT NULL);"
				}
			...
			if backUpType == 1 {
				if strings.Contains(query, "CREATE TABLE IF NOT EXISTS") {
					query = strings.Replace(query, "AUTOINCREMENT", "AUTO_INCREMENT", -1)
				} else if strings.Contains(query, "CREATE TABLE IF NOT EXISTS `display_model_layer`") {
					query = "CREATE TABLE IF NOT EXISTS `display_model_layer` (`id` INT PRIMARY KEY AUTO_INCREMENT NOT NULL, `created_at` DATETIME(3) NULL, `updated_at` DATETIME(3) NULL, `deleted_at` DATETIME(3) NULL, `model_id` VARCHAR(250) NOT NULL, `page_name` VARCHAR(250) NOT NULL, `page_id` VARCHAR(250) NOT NULL, `is_home` INT NOT NULL,`is_login` INT NOT NULL, `page_type` INT NOT NULL, `layer` LONGTEXT NOT NULL, `components` LONGTEXT NOT NULL) ENGINE=InnoDB DEFAULT CHARSET utf8;"
				}
			}
