...
var (
	rgxName     = regexp.MustCompile(`\{\{\s*\w+\.(\w+)\s*\}\}`)
...
func renderRows(sheet *xlsx.Sheet, rows []*xlsx.Row, ctx map[string]interface{}, options *Options) error {
	for ri := 0; ri < len(rows); ri++ {
		...
		for i := 0; i < arr.Len(); i++ {
			...
			if len(ctx[prop].(map[string]interface{})) == 0 {
				continue
			}
			newRow := sheet.AddRow()
			cloneRow(row, newRow, options)
...
func (m *Xlst) GetAllPropName() []string {

	var dataName []string
	for _, sheet := range m.file.Sheets {
		for ri := 0; ri < len(sheet.Rows); ri++ {
			row := sheet.Rows[ri]
			for _, cell := range row.Cells {
				if cell.Value == "" {
					continue
				}
				result := strings.Split(cell.Value, ".")
				if len(result) == 2 {
					result2 := strings.Split(result[1], "}}")
					name := result2[0]
					result3 := strings.Split(name, "的")
					if len(result3) == 2 {
						dataName = append(dataName, result3[0])
					} else {
						if name != "HistoryRecordDateTime" {
							dataName = append(dataName, name)
						}
					}
				}
			}
