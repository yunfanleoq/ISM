...
func OpcuaNodeEdit(muid, uuid string, data OpcuaDevicesDataModel) int {
	...
	result := Db.Model(&OpcuaDevicesDataModel{}).Select("description", "type", "nodeid", "data_unit", "conversion_expression", "name", "auth", "is_alarm", "record_data_charge", "record_type", "is_record", "record_interval", "alarm_level", "alarm_message", "alarm_clear_message").Where("uuid = ?", uuid).Updates(data)
	if result.Error != nil {
		return errmsg.SNMP_MODEL_ADD_FAILED
	}
