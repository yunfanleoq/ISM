...
// OPCUA模型结构体
type OpcuaDevicesDataModel struct {
	gorm.Model
...
var OPCUANodeidPath string = "data/upload/"

// 模型添加
func OpcuaModelAdd(params DevicesModel) int {
...
// 模型列表获取
func OpcuaModelList(DataModelType int, ProjectUuid string) ([]DevicesModel, int64) {
...
// 模型删除
func OpcuaModelDel(key string) int {
...
// 模型更新
func OpcuaModelUpdate(key string, data DevicesModel) int {
...
// OPCUA nodeid 添加
func OpcuaNodeAdd(data OpcuaDevicesDataModel) int {
	var getDeviceData []MonitorList
	var getOpcuaDevicesDataModel OpcuaDevicesDataModel

	existData := Db.Model(&OpcuaDevicesDataModel{}).Where("(name = ? or nodeid = ?) and muid = ?", data.Name, data.Nodeid, data.Muid).First(&getOpcuaDevicesDataModel)
	if !errors.Is(existData.Error, gorm.ErrRecordNotFound) {
		return errmsg.SNMP_MODEL_EXIST
	}
	data.Uuid = uuid.New()
	result := Db.Model(&OpcuaDevicesDataModel{}).Create(&data)
	if result.Error != nil {
		return errmsg.SNMP_MODEL_ADD_FAILED
	}
	...
	if !errors.Is(existError.Error, gorm.ErrRecordNotFound) {
		...
		for _, v := range getDeviceData {
			...
			writeDeviceRealData[k].DeviceType = 3
			k++
		}
...
func OpcuaNodeList(muid string) []OpcuaDevicesDataModel {
	var getModelData []OpcuaDevicesDataModel
	Db.Model(&OpcuaDevicesDataModel{}).Where("muid = ?", muid).Find(&getModelData)
	return getModelData
}

func OpcuaNodeDel(uuid string) int {
	var delModelData OpcuaDevicesDataModel
	...
	err := Db.Model(&OpcuaDevicesDataModel{}).Unscoped().Where("uuid = ?", uuid).Delete(&delModelData).Error
	if err != nil {
		return errmsg.ERROR
	}
	return errmsg.SUCCSE
...
func OpcuaNodeEdit(muid, uuid string, data OpcuaDevicesDataModel) int {
	var getOpcuaDevicesDataModel OpcuaDevicesDataModel
	var updateRealData DeviceRealData

	existData := Db.Model(&OpcuaDevicesDataModel{}).Where("(name = ? or nodeid = ?) and uuid != ? and muid=?", data.Name, data.Nodeid, uuid, muid).First(&getOpcuaDevicesDataModel).Error
	if existData != gorm.ErrRecordNotFound {
		return errmsg.SNMP_MODEL_EXIST
	}

	result := Db.Model(&OpcuaDevicesDataModel{}).Select("description", "type", "nodeid", "data_unit", "conversion_expression", "name", "auth", "is_alarm", "alarm_confirm_delay_sec", "record_data_charge", "record_type", "is_record", "record_interval", "alarm_level", "alarm_message", "alarm_clear_message").Where("uuid = ?", uuid).Updates(data)
	if result.Error != nil {
		return errmsg.SNMP_MODEL_ADD_FAILED
	}
