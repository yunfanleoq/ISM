...
// Modbus模型结构体
type IEC104DevicesDataModel struct {
	gorm.Model
...
// 模型删除
func IEC104ModelDel(key string) int {
	...
	err = Db.Model(&IEC104DevicesDataModel{}).Unscoped().Where("muid = ?", key).Delete(&IEC104DevicesDataModel{}).Error
	if err != nil {
		return errmsg.ERROR
	}
...
// OPCUA nodeid 添加
func IEC104NodeAdd(data IEC104DevicesDataModel) int {
	var getDeviceData []MonitorList
	var getOpcuaDevicesDataModel IEC104DevicesDataModel

	existData := Db.Model(&IEC104DevicesDataModel{}).Where("(name = ? or data_point = ?) and muid = ?", data.Name, data.DataPoint, data.Muid).First(&getOpcuaDevicesDataModel)
	if !errors.Is(existData.Error, gorm.ErrRecordNotFound) {
		return errmsg.SNMP_MODEL_EXIST
	}
	data.Uuid = uuid.New()
	result := Db.Model(&IEC104DevicesDataModel{}).Create(&data)
	if result.Error != nil {
	...
	if !errors.Is(existError.Error, gorm.ErrRecordNotFound) {
		...
		for _, v := range getDeviceData {
			...
			writeDeviceRealData[k].DeviceType = 40
			k++
		}
...
func IEC104NodeList(muid string) []IEC104DevicesDataModel {
	var getModelData []IEC104DevicesDataModel
	Db.Model(&IEC104DevicesDataModel{}).Where("muid = ?", muid).Find(&getModelData)
	return getModelData
}

func IEC104NodeDel(uuid string) int {
	...
	err := Db.Model(&IEC104DevicesDataModel{}).Unscoped().Where("uuid = ?", uuid).Delete(&delModelData).Error
	if err != nil {
		return errmsg.ERROR
	}
	return errmsg.SUCCSE
...
func IEC104NodeEdit(muid, uuid string, data IEC104DevicesDataModel) int {
	var getOpcuaDevicesDataModel IEC104DevicesDataModel
	var updateRealData DeviceRealData

	existData := Db.Model(&IEC104DevicesDataModel{}).Where("(name = ? or data_point = ?) and uuid != ? and muid=?", data.Name, data.DataPoint, uuid, muid).First(&getOpcuaDevicesDataModel).Error
