...
// bacnet模型结构体
type BacnetDevicesDataModel struct {
	gorm.Model

	Name                 string `gorm:"index;type:varchar(250);not null"  json:"name" validate:"required,min=4,max=250" label:"名称"`
	BacnetZone           int    `gorm:"type:int;" json:"BacnetZone" validate:"required" label:"读写的区域"`
	BacnetAddress        int    `gorm:"type:int;" json:"BacnetAddress" validate:"required" label:"读写的地址"`
...
// 模型添加
func BACnetModelAdd(params DevicesModel) int {
...
// 模型列表获取
func BACnetModelList(DataModelType int, ProjectUuid string) ([]DevicesModel, int64) {
...
// 模型删除
func BACnetModelDel(key string) int {
...
// 模型更新
func BACnetModelUpdate(key string, data DevicesModel) int {
...
// OPCUA nodeid 添加
func BACnetNodeAdd(data BacnetDevicesDataModel) int {
	var getDeviceData []MonitorList
	var getBacnetDevicesDataModel BacnetDevicesDataModel

	existData := Db.Model(&BacnetDevicesDataModel{}).Where("name = ?  and muid = ?", data.Name, data.Muid).First(&getBacnetDevicesDataModel)
	if !errors.Is(existData.Error, gorm.ErrRecordNotFound) {
		return errmsg.SNMP_MODEL_EXIST
	}
	data.Uuid = uuid.New()
	result := Db.Model(&BacnetDevicesDataModel{}).Create(&data)
	if result.Error != nil {
		return errmsg.SNMP_MODEL_ADD_FAILED
	}
	...
	if !errors.Is(existError.Error, gorm.ErrRecordNotFound) {
		...
		for _, v := range getDeviceData {
			...
			if data.Type == "2" || data.Type == "1" { //INT
				writeDeviceRealData[k].Type = 1
			} else if data.Type == "3" { //Float
			...
			writeDeviceRealData[k].DeviceType = 500
			k++
		}
...
func BACnetNodeList(muid string) []BacnetDevicesDataModel {
	var getModelData []BacnetDevicesDataModel
	Db.Model(&BacnetDevicesDataModel{}).Where("muid = ?", muid).Find(&getModelData)
	return getModelData
}

func BACnetNodeDel(uuid string) int {
	var delModelData BacnetDevicesDataModel
	...
	err := Db.Model(&BacnetDevicesDataModel{}).Unscoped().Where("uuid = ?", uuid).Delete(&delModelData).Error
	if err != nil {
		return errmsg.ERROR
	}
	return errmsg.SUCCSE
...
func BACnetNodeEdit(muid, uuid string, data BacnetDevicesDataModel) int {
	var getBacnetDevicesDataModel BacnetDevicesDataModel
	var updateRealData DeviceRealData

	existData := Db.Model(&BacnetDevicesDataModel{}).Where("name = ? and uuid != ? and muid=?", data.Name, uuid, muid).First(&getBacnetDevicesDataModel).Error
	if existData != gorm.ErrRecordNotFound {
		return errmsg.SNMP_MODEL_EXIST
	}

	result := Db.Model(&BacnetDevicesDataModel{}).Select("description", "type", "bacnet_zone", "bacnet_address", "data_unit", "conversion_expression", "name", "auth", "is_alarm", "alarm_confirm_delay_sec", "record_data_charge", "record_type", "is_record", "record_interval", "alarm_level", "alarm_message", "alarm_clear_message").Where("uuid = ?", uuid).Updates(data)
	if result.Error != nil {
		return errmsg.SNMP_MODEL_ADD_FAILED
	}
