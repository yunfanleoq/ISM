...
func BACnetNodeEdit(muid, uuid string, data BacnetDevicesDataModel) int {
	...
	result := Db.Model(&BacnetDevicesDataModel{}).Select("description", "type", "bacnet_zone", "bacnet_address", "data_unit", "conversion_expression", "name", "auth", "is_alarm", "record_data_charge", "record_type", "is_record", "record_interval", "alarm_level", "alarm_message", "alarm_clear_message").Where("uuid = ?", uuid).Updates(data)
	if result.Error != nil {
		return errmsg.SNMP_MODEL_ADD_FAILED
	}
