...
// OPCUA模型结构体
type VirtualDeviceDataModel struct {
	gorm.Model
...
// 模型添加
func VirtualDeviceModelAdd(params DevicesModel) (int, string) {
...
// 模型列表获取
func VirtualDeviceModelList(DataModelType int, ProjectUuid string) ([]DevicesModel, int64) {
...
// 模型删除
func VirtualDeviceModelDel(key string) int {
...
// 模型更新
func VirtualDeviceModelUpdate(key string, data DevicesModel) int {
...
// 添加
func VirtualDeviceDataAdd(data VirtualDeviceDataModel) int {
	var getDeviceData []MonitorList
	var getOpcuaDevicesDataModel VirtualDeviceDataModel

	existData := Db.Model(&VirtualDeviceDataModel{}).Where("name = ? and muid = ?", data.Name, data.Muid).First(&getOpcuaDevicesDataModel)
	if !errors.Is(existData.Error, gorm.ErrRecordNotFound) {
		return errmsg.SNMP_MODEL_EXIST
	}
	data.Uuid = uuid.New()
	result := Db.Model(&VirtualDeviceDataModel{}).Create(&data)
	if result.Error != nil {
	...
	if !errors.Is(existError.Error, gorm.ErrRecordNotFound) {
		...
		for _, v := range getDeviceData {
			...
			writeDeviceRealData[k].DeviceType = 480
			k++
		}
...
func VirtualDeviceDataEdit(muid, uuid string, data VirtualDeviceDataModel) int {
	var getOpcuaDevicesDataModel VirtualDeviceDataModel
	var updateRealData DeviceRealData

	existData := Db.Model(&VirtualDeviceDataModel{}).Where("name = ?  and uuid != ? and muid=?", data.Name, uuid, muid).First(&getOpcuaDevicesDataModel).Error
	if existData != gorm.ErrRecordNotFound {
		return errmsg.SNMP_MODEL_EXIST
	}

	result := Db.Model(&VirtualDeviceDataModel{}).Select("type", "data_unit", "conversion_expression", "name", "auth", "is_alarm", "alarm_on_value", "alarm_confirm_delay_sec", "record_data_charge", "record_type", "is_record", "record_interval", "alarm_level", "alarm_message", "alarm_clear_message").Where("uuid = ?", uuid).Updates(data)
	if result.Error != nil {
		return errmsg.SNMP_MODEL_ADD_FAILED
	}
	...
	err := Db.Model(&DeviceRealData{}).Select("data_unit", "conversion_expression", "name", "auth", "is_alarm", "alarm_on_value", "alarm_confirm_delay_sec", "record_type", "record_data_charge", "is_record", "record_interval", "alarm_level", "alarm_message", "alarm_clear_message").Where("model_data_uuid = ?", uuid).Updates(updateRealData).Error
	if err != nil {
		return errmsg.ERROR
	}
	return errmsg.SNMP_MODEL_ADD_SUCCSE
...
func VirtualDeviceDataDel(uuid string) int {
	var delModelData VirtualDeviceDataModel
	...
	err := Db.Model(&VirtualDeviceDataModel{}).Unscoped().Where("uuid = ?", uuid).Delete(&delModelData).Error
	if err != nil {
		return errmsg.ERROR
	}
	return errmsg.SUCCSE
...
func VirtualDeviceDataList(muid string) []VirtualDeviceDataModel {
	var getModelData []VirtualDeviceDataModel
	Db.Model(&VirtualDeviceDataModel{}).Where("muid = ?", muid).Find(&getModelData)
	return getModelData
}
