 ...
 * @ Modified time: 2023-04-03 08:59:19
...
// OPCUA模型结构体
type SimS7DataModel struct {
	gorm.Model

	Name                 string `gorm:"index;type:varchar(250);not null"  json:"name" validate:"required,min=4,max=250" label:"名称"`
	DataFromType         int    `gorm:"type:int;not null" json:"DataFromType" validate:"required,min=2,max=250" label:"数据来源"`
	DBIndex              int    `gorm:"index;type:int;not null" json:"DBIndex" validate:"required,min=2,max=250" label:"数据块序号"`
	DBOffset             string `gorm:"type:varchar(250);not null" json:"DBOffset" validate:"required,min=2,max=250" label:"数据块偏移量"`
	...
	StringMaxLength      int    `gorm:"type:int;" json:"StringMaxLength" validate:"required" label:"字符串类型最大长度"`
	IsHaveUnsigned       int    `gorm:"type:int;" json:"IsHaveUnsigned" validate:"required" label:"是否有符号"`
...
// 模型添加
func SimS7ModelAdd(params DevicesModel) int {
	var getModel DevicesModel

	Db.Where("name = ? and project_uuid = ? and type=15", params.Name, params.ProjectUuid).First(&getModel)
	if getModel.ID != 0 {
...
// 模型列表获取
func SimS7ModelList(DataModelType int, ProjectUuid string) ([]DevicesModel, int64) {
...
// 模型删除
func SimS7ModelDel(key string) int {
...
// 模型更新
func SimS7ModelUpdate(key string, data DevicesModel) int {
...
// 添加
func SimS7ModelDataAdd(data SimS7DataModel) int {
	var getDeviceData []MonitorList
	var getOpcuaDevicesDataModel SimS7DataModel

	existData := Db.Model(&SimS7DataModel{}).Where("name = ? and muid = ?", data.Name, data.Muid).First(&getOpcuaDevicesDataModel)
	if !errors.Is(existData.Error, gorm.ErrRecordNotFound) {
		return errmsg.SNMP_MODEL_EXIST
	}
	data.Uuid = uuid.New()
	result := Db.Model(&SimS7DataModel{}).Create(&data)
	if result.Error != nil {
		return errmsg.SNMP_MODEL_ADD_FAILED
	}
	...
	if !errors.Is(existError.Error, gorm.ErrRecordNotFound) {
		...
		for _, v := range getDeviceData {
			...
			writeDeviceRealData[k].DeviceType = 15
			k++
		}
...
func SimS7ModelDataEdit(muid, uuid string, data SimS7DataModel) int {
	var getOpcuaDevicesDataModel SimS7DataModel
	var updateRealData DeviceRealData

	existData := Db.Model(&SimS7DataModel{}).Where("name = ?  and uuid != ? and muid=?", data.Name, uuid, muid).First(&getOpcuaDevicesDataModel).Error
	if existData != gorm.ErrRecordNotFound {
		return errmsg.SNMP_MODEL_EXIST
	}

	result := Db.Model(&SimS7DataModel{}).Select("is_have_unsigned", "string_max_length", "db_offset", "db_index", "data_from_type", "type", "data_unit", "conversion_expression", "name", "auth", "is_alarm", "alarm_confirm_delay_sec", "record_data_charge", "record_type", "is_record", "record_interval", "alarm_level", "alarm_message", "alarm_clear_message", "description").Where("uuid = ?", uuid).Updates(data)
	if result.Error != nil {
		return errmsg.SNMP_MODEL_ADD_FAILED
	}
...
func SimS7ModelDataDel(uuid string) int {
	var delModelData SimS7DataModel
	...
	err := Db.Model(&SimS7DataModel{}).Unscoped().Where("uuid = ?", uuid).Delete(&delModelData).Error
	if err != nil {
		return errmsg.ERROR
	}
	return errmsg.SUCCSE
...
func SimS7ModelDataList(muid string) []SimS7DataModel {
	var getModelData []SimS7DataModel
	Db.Model(&SimS7DataModel{}).Where("muid = ?", muid).Find(&getModelData)
	return getModelData
}
