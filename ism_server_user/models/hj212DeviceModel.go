...
// OPCUA模型结构体
type HJ212DevicesDataModel struct {
	gorm.Model

	Name                 string `gorm:"index;type:varchar(250);not null"  json:"name" validate:"required,min=4,max=250" label:"名称"`
	EncodeID             string `gorm:"index;type:varchar(250);not null" json:"EncodeID" validate:"required,min=2,max=250" label:"编码"`
...
// 模型添加
func HJ212ModelAdd(params DevicesModel) int {
...
// 模型列表获取
func HJ212ModelList(DataModelType int, ProjectUuid string) ([]DevicesModel, int64) {
...
// 模型删除
func HJ212ModelDel(key string) int {
...
// 模型更新
func HJ212ModelUpdate(key string, data DevicesModel) int {
...
// OPCUA nodeid 添加
func HJ212NodeAdd(data HJ212DevicesDataModel) int {
	var getDeviceData []MonitorList
	var getMqttDevicesDataModel HJ212DevicesDataModel

	existData := Db.Model(&HJ212DevicesDataModel{}).Where("(name = ? or encode_id = ?) and muid = ?", data.Name, data.EncodeID, data.Muid).First(&getMqttDevicesDataModel)
	if !errors.Is(existData.Error, gorm.ErrRecordNotFound) {
		return errmsg.SNMP_MODEL_EXIST
	}
	data.Uuid = uuid.New()
	result := Db.Model(&HJ212DevicesDataModel{}).Create(&data)
	if result.Error != nil {
		return errmsg.SNMP_MODEL_ADD_FAILED
	}
	...
	if !errors.Is(existError.Error, gorm.ErrRecordNotFound) {
		...
		for _, v := range getDeviceData {
			...
			writeDeviceRealData[k].DeviceType = 470
			k++
		}
...
func HJ212NodeList(muid string) []HJ212DevicesDataModel {
	var getModelData []HJ212DevicesDataModel
	Db.Model(&HJ212DevicesDataModel{}).Where("muid = ?", muid).Find(&getModelData)
	return getModelData
}

func HJ212NodeDel(uuid string) int {
	var delModelData HJ212DevicesDataModel
	...
	err := Db.Model(&HJ212DevicesDataModel{}).Unscoped().Where("uuid = ?", uuid).Delete(&delModelData).Error
	if err != nil {
		return errmsg.ERROR
	}
	return errmsg.SUCCSE
...
func HJ212NodeEdit(muid, uuid string, data HJ212DevicesDataModel) int {
	var getMqttDevicesDataModel HJ212DevicesDataModel
	var updateRealData DeviceRealData

	existData := Db.Model(&HJ212DevicesDataModel{}).Where("(name = ? or encode_id = ?) and uuid != ? and muid=?", data.Name, data.EncodeID, uuid, muid).First(&getMqttDevicesDataModel).Error
	if existData != gorm.ErrRecordNotFound {
		return errmsg.SNMP_MODEL_EXIST
	}

	result := Db.Model(&HJ212DevicesDataModel{}).Select("description", "type", "encode_id", "data_unit", "conversion_expression", "name", "auth", "is_alarm", "alarm_confirm_delay_sec", "record_data_charge", "record_type", "is_record", "record_interval", "alarm_level", "alarm_message", "alarm_clear_message").Where("uuid = ?", uuid).Updates(data)
	if result.Error != nil {
		return errmsg.SNMP_MODEL_ADD_FAILED
	}
