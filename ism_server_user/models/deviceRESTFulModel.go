...
func RESTFulDataEdit(muid, uuid string, data RESTFulDataModel) int {
	...
	result := Db.Model(&RESTFulDataModel{}).Select("type", "data_unit", "conversion_expression", "name", "auth", "is_alarm", "alarm_confirm_delay_sec", "record_data_charge", "record_type", "is_record", "record_interval", "alarm_level", "alarm_message", "alarm_clear_message").Where("uuid = ?", uuid).Updates(data)
	if result.Error != nil {
		return errmsg.SNMP_MODEL_ADD_FAILED
	}
