...
// DLT645模型结构体
type Dlt645DevicesDataModel struct {
	gorm.Model
...
// 模型删除
func Dlt645ModelDel(key string) int {
	...
	err = Db.Model(&Dlt645DevicesDataModel{}).Unscoped().Where("muid = ?", key).Delete(&Dlt645DevicesDataModel{}).Error
	if err != nil {
		return errmsg.ERROR
	}
...
// OPCUA nodeid 添加
func DLT645NodeAdd(data Dlt645DevicesDataModel) int {
	var getDeviceData []MonitorList
	var getOpcuaDevicesDataModel Dlt645DevicesDataModel

	existData := Db.Model(&Dlt645DevicesDataModel{}).Where("(name = ? or data_identification = ?) and muid = ?", data.Name, data.DataIdentification, data.Muid).First(&getOpcuaDevicesDataModel)
	if !errors.Is(existData.Error, gorm.ErrRecordNotFound) {
		return errmsg.SNMP_MODEL_EXIST
	}
	data.Uuid = uuid.New()
	result := Db.Model(&Dlt645DevicesDataModel{}).Create(&data)
	if result.Error != nil {
...
func DLT645NodeList(muid string) []Dlt645DevicesDataModel {
	var getModelData []Dlt645DevicesDataModel
	Db.Model(&Dlt645DevicesDataModel{}).Where("muid = ?", muid).Find(&getModelData)
	return getModelData
}

func DLT645NodeDel(uuid string) int {
	var delModelData Dlt645DevicesDataModel
	...
	err := Db.Model(&Dlt645DevicesDataModel{}).Unscoped().Where("uuid = ?", uuid).Delete(&delModelData).Error
	if err != nil {
		return errmsg.ERROR
	}
	return errmsg.SUCCSE
...
func DLT645NodeEdit(muid, uuid string, data Dlt645DevicesDataModel) int {
	var getOpcuaDevicesDataModel Dlt645DevicesDataModel
	var updateRealData DeviceRealData

	existData := Db.Model(&Dlt645DevicesDataModel{}).Where("(name = ? or data_identification = ?) and uuid != ? and muid=?", data.Name, data.DataIdentification, uuid, muid).First(&getOpcuaDevicesDataModel).Error
	if existData != gorm.ErrRecordNotFound {
		return errmsg.SNMP_MODEL_EXIST
	}

	result := Db.Model(&Dlt645DevicesDataModel{}).Select("record_data_timely", "description", "type", "data_identification", "data_unit", "conversion_expression", "name", "auth", "is_alarm", "alarm_confirm_delay_sec", "record_data_charge", "record_type", "is_record", "record_interval", "alarm_level", "alarm_message", "alarm_clear_message").Where("uuid = ?", uuid).Updates(data)
	if result.Error != nil {
		return errmsg.SNMP_MODEL_ADD_FAILED
	}
	...
	err := Db.Model(&DeviceRealData{}).Select("record_data_timely", "data_unit", "conversion_expression", "name", "auth", "is_alarm", "alarm_confirm_delay_sec", "record_type", "record_data_charge", "is_record", "record_interval", "alarm_level", "alarm_message", "alarm_clear_message").Where("model_data_uuid = ?", uuid).Updates(updateRealData).Error
	if err != nil {
		return errmsg.ERROR
	}
	return errmsg.SNMP_MODEL_ADD_SUCCSE
