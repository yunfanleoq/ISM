 ...
 * @ Modified time: 2023-08-10 17:15:40
...
// OPCUA模型结构体
type IEC61850DevicesDataModel struct {
	gorm.Model
	...
	FunType              string `gorm:"type:varchar(250);not null" json:"FunType" validate:"required" label:"功能类型"`
...
var IEC61850NodeidPath string = "data/upload/"

// 模型添加
func IEC61850ModelAdd(params DevicesModel) int {
...
// 模型列表获取
func IEC61850ModelList(DataModelType int, ProjectUuid string) ([]DevicesModel, int64) {
...
// 模型删除
func IEC61850ModelDel(key string) int {
...
// 模型更新
func IEC61850ModelUpdate(key string, data DevicesModel) int {
...
// OPCUA nodeid 添加
func IEC61850NodeAdd(data IEC61850DevicesDataModel) int {
	var getDeviceData []MonitorList
	var getOpcuaDevicesDataModel IEC61850DevicesDataModel

	existData := Db.Model(&IEC61850DevicesDataModel{}).Where("(name = ? or nodeid = ?) and muid = ?", data.Name, data.Nodeid, data.Muid).First(&getOpcuaDevicesDataModel)
	if !errors.Is(existData.Error, gorm.ErrRecordNotFound) {
		return errmsg.SNMP_MODEL_EXIST
	}
	data.Uuid = uuid.New()
	result := Db.Model(&IEC61850DevicesDataModel{}).Create(&data)
	if result.Error != nil {
		return errmsg.SNMP_MODEL_ADD_FAILED
	}
	...
	if !errors.Is(existError.Error, gorm.ErrRecordNotFound) {
		...
		for _, v := range getDeviceData {
			...
			writeDeviceRealData[k].DeviceType = 350
			k++
		}
...
func IEC61850NodeList(muid string) []IEC61850DevicesDataModel {
	var getModelData []IEC61850DevicesDataModel
	Db.Model(&IEC61850DevicesDataModel{}).Where("muid = ?", muid).Find(&getModelData)
	return getModelData
}

func IEC61850NodeDel(uuid string) int {
	var delModelData IEC61850DevicesDataModel
	...
	err := Db.Model(&IEC61850DevicesDataModel{}).Unscoped().Where("uuid = ?", uuid).Delete(&delModelData).Error
	if err != nil {
		return errmsg.ERROR
	}
	return errmsg.SUCCSE
}
func IEC61850DelAllNodes(muid string) int {
	var delModelData IEC61850DevicesDataModel
	var getBandDevice DeviceRealData

	var getAllNodes []IEC61850DevicesDataModel

	err := Db.Model(&IEC61850DevicesDataModel{}).Where("muid = ?", muid).Find(&getAllNodes).Error
	if err == nil {
		for _, v := range getAllNodes {
			err1 := Db.Unscoped().Model(&DeviceRealData{}).Where("model_data_uuid = ?", v.Uuid).Delete(&getBandDevice).Error
			if err1 != nil {
				return errmsg.MODEL_HAVED_BAND
			}

			err = Db.Model(&IEC61850DevicesDataModel{}).Unscoped().Where("uuid = ?", v.Uuid).Delete(&delModelData).Error
			if err != nil {
				return errmsg.ERROR
			}
		}
...
func IEC61850NodeEdit(muid, uuid string, data IEC61850DevicesDataModel) int {
	var getOpcuaDevicesDataModel IEC61850DevicesDataModel
	var updateRealData DeviceRealData

	existData := Db.Model(&IEC61850DevicesDataModel{}).Where("(name = ? or nodeid = ?) and uuid != ? and muid=?", data.Name, data.Nodeid, uuid, muid).First(&getOpcuaDevicesDataModel).Error
	if existData != gorm.ErrRecordNotFound {
		return errmsg.SNMP_MODEL_EXIST
	}

	result := Db.Model(&IEC61850DevicesDataModel{}).Select("description", "type", "nodeid", "data_unit", "conversion_expression", "name", "auth", "is_alarm", "alarm_confirm_delay_sec", "record_data_charge", "record_type", "is_record", "record_interval", "alarm_level", "alarm_message", "alarm_clear_message").Where("uuid = ?", uuid).Updates(data)
	if result.Error != nil {
		return errmsg.SNMP_MODEL_ADD_FAILED
	}
