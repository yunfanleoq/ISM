...
//
//	*Address 设备地址
func ToBigEndian(data []byte) []byte {
	// 创建等长切片
...
func NewAddress(address string, order Order) Address {
	...
	strValue := Bcd2Number(addrBytes)
	value := ToBigEndian(addrBytes)
	return Address{value: value, strValue: strValue}
}
...
func (p Protocol) Encode(buffer *bytes.Buffer) error {
	...
	write := func(data interface{}) {
		...
		err = binary.Write(bf, binary.BigEndian, data)
	}
	write(&p.Start)
	write(&p.MeterType)
	...
	write(p.CS)
	write(p.End)
	err = binary.Write(buffer, binary.BigEndian, bf.Bytes())
	return err
...
func Bytes2Int(data []byte) uint16 {
	var dateCode uint16 = 0
	var dataCode3 uint16 = uint16(data[1])
	var dataCode4 uint16 = uint16(data[0])
	dateCode = (dataCode3) | (dataCode4 << 8)
	return dateCode
}
