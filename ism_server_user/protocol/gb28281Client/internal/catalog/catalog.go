...
func (catalog *Catalog) DeviceInfoHandle(xlog *xlog.Logger, tr *transport.Transport, req *sip.Msg) {
	...
	deviceInfo := catalog.sendDeviceInfoResp(xlog, laHost, laPort, q.SN)
	go func() {
		tr.Send <- deviceInfo
	}()
}
...
func (catalog *Catalog) sendDeviceInfoResp(xlog *xlog.Logger, localHost string, localPort int, sn string) *sip.Msg {
	...
	req.Payload = &deviceInfo{
		...
		Model:        "Model",
		Firmware:     "Firmware",
		Channel:      strconv.Itoa(len(catalog.cfg.Devices)),
	}
	return req
}
...
func (cataInfo *deviceInfo) ContentType() string {
	return "Application/MANSCDP+xml"
}
func (cataInfo *deviceInfo) Data() []byte {
	data, _ := xml.MarshalIndent(cataInfo, "  ", "    ")
	return []byte(xml.Header + string(data))
}
