...
import (
	...
	"ISMServer/protocol/gb28281Client/internal/catalog"
	"ISMServer/protocol/gb28281Client/internal/config"
	"ISMServer/protocol/gb28281Client/internal/invite"
	"ISMServer/protocol/gb28281Client/internal/reg"
...
func (s *Service) HandleIncommingMsg() {
	...
	for m := range s.tr.Recv {
		...
		if !m.IsResponse() && m.CSeqMethod == sip.MethodMessage {
			...
			case DeviceInfo:
				s.catalogSrv.DeviceInfoHandle(s.xlog, s.tr, m)
			case Unknow:
