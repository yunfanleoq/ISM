#!/bin/bash
# 为 MySQL/OceanBase 业务库补全 template_kind 列，并绑定「设备列表」模板页
...
PAGE_ID="${DEVICE_LIST_TEMPLATE_PAGE_ID:?必须显式提供 DEVICE_LIST_TEMPLATE_PAGE_ID}"
...
SELECT NOW(6), NOW(6), NULL, '${MODEL_ID}', '${PAGE_NAME}', '${PAGE_ID}', 0, 0, 1, '', '${COMPONENTS_B64}', 'deviceList', ''
WHERE NOT EXISTS (
...
SET page_name='${PAGE_NAME}', template_kind='deviceList', template_model_uuid='', updated_at=NOW(6), deleted_at=NULL
