...
var Tabs = /** @class */ (function (_super) {
            ...
            if (tab) {
                ...
                activeKey =
                    typeof props.defaultKey === 'string'
                        ? amisCore.resolveVariableAndFilter(props.defaultKey, props.data)
                        : props.defaultKey;
            }
            else if (props.defaultActiveKey) {
                activeKey = amisCore.resolveVariableAndFilter(props.defaultActiveKey, props.data);
            }
            activeKey = activeKey || (tabs[0] && tabs[0].hash) || 0;
        }
        var _a = tslib.__read(_this.initTabArray(tabs, source, data), 2), localTabs = _a[0], isFromSource = _a[1];
        _this.state = {
            prevKey: undefined,
    ...
    Tabs.prototype.initTabArray = function (tabs, source, data) {
        ...
        arr.forEach(function (value) {
            sourceTabs.push.apply(sourceTabs, tslib.__spreadArray([], tslib.__read(tabs.map(function (tab) { return (tslib.__assign(tslib.__assign({}, tab), { data: value })); })), false));
        });
    ...
    Tabs.prototype.componentDidUpdate = function (preProps, prevState) {
        ...
        var localTabs = this.state.localTabs;
        var prevActiveKey = amisCore.resolveVariableAndFilter(preProps.defaultActiveKey, preProps.data);
        var activeKey = amisCore.resolveVariableAndFilter(props.defaultActiveKey, props.data);
        // 响应外部修改 tabs
        ...
        if (isTabsModified) {
            var _d = tslib.__read(this.initTabArray(props.tabs, props.source, props.data), 2), newLocalTabs = _d[0], isFromSource = _d[1];
            this.setState({
    ...
    Tabs.prototype.autoJumpToNeighbour = function (key) {
        ...
        if (localTabs[tabIndex] &&
            ...
            while (left-- >= 0 || right++ < len) {
                var activeKey = null;
                if (left >= 0 && amisCore.isVisible(localTabs[left], data)) {
                    activeKey = localTabs[left].hash || left;
                }
                else if (right < len && amisCore.isVisible(localTabs[right], data)) {
    ...
    Tabs.prototype.doAction = function (action, args) {
        ...
        var activeKey = isNaN__default["default"](tmpKey) ? args === null || args === void 0 ? void 0 : args.activeKey : tmpKey;
    ...
    Tabs.prototype.renderTabTitle = function (title, index, data) {
        ...
        return typeof title === 'string' || !title
            ? amisCore.filter(title, data)
            : render("tab-title/".concat(index), title, tslib.__assign(tslib.__assign({}, data), { index: index }));
    };
    Tabs.prototype.renderToolbar = function () {
        ...
        return toolbar ? (_J$X_("div", { className: cx("Tabs-toolbar", toolbarClassName) }, render('toolbar', toolbar))) : null;
    };
    Tabs.prototype.renderTabs = function () {
        ...
        if (isFromSource) {
            children = tabs.map(function (tab, index) {
                var _a, _b;
                var ctx = amisCore.createObject(data, amisCore.isObject(tab.data) ? tslib.__assign({ index: index }, tab.data) : { item: tab.data, index: index });
                return amisCore.isVisible(tab, ctx) ? (_J$X_(amisUi.Tab, tslib.__assign({}, tab, { title: _this.renderTabTitle(tab.title, index, ctx), disabled: disabled || amisCore.isDisabled(tab, ctx), key: index, eventKey: amisCore.filter(tab.hash, ctx) || index, prevKey: index > 0 ? ((_a = tabs[index - 1]) === null || _a === void 0 ? void 0 : _a.hash) || index - 1 : 0, nextKey: index < tabs.length - 1
                    ...
                    disabled: disabled || amisCore.isDisabled(tab, ctx) || undefined,
                    data: ctx,
            ...
            children = tabs.map(function (tab, index) {
                var _a, _b;
                return amisCore.isVisible(tab, data) ? (_J$X_(amisUi.Tab, tslib.__assign({}, tab, { title: _this.renderTabTitle(tab.title, index, data), disabled: disabled || amisCore.isDisabled(tab, data), key: index, eventKey: tab.hash || index, prevKey: index > 0 ? ((_a = tabs[index - 1]) === null || _a === void 0 ? void 0 : _a.hash) || index - 1 : 0, nextKey: index < tabs.length - 1
                        ? ((_b = tabs[index + 1]) === null || _b === void 0 ? void 0 : _b.hash) || index + 1
                        : tabs.length - 1, swipeable: swipeable, mobileUI: mobileUI, mountOnEnter: mountOnEnter, unmountOnExit: typeof tab.reload === 'boolean'
                        ? tab.reload
                        : typeof tab.unmountOnExit === 'boolean'
