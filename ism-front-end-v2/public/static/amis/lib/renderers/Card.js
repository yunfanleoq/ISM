...
var CardRenderer = /** @class */ (function (_super) {
    ...
    CardRenderer.prototype.renderToolbar = function () {
        ...
        if (header) {
            ...
            if (typeof highlight === 'string'
                ...
                : highlight) {
                toolbars.push(_J$X_("i", { key: "highlight", className: cx('Card-highlight', highlightClassName) }));
            }
        }
        if (selectable && !hideCheckToggler) {
            toolbars.push(_J$X_(amisUi.Checkbox, { key: "check", className: cx('Card-checkbox'), type: multiple !== false ? 'checkbox' : 'radio', disabled: !checkable, checked: selected, onChange: this.handleCheck }));
        }
        ...
        if (dragging) {
            toolbars.push(_J$X_("div", { className: cx('Card-dragBtn') },
                _J$X_(amisUi.Icon, { icon: "drag-bar", className: "icon" })));
        }
        return toolbars.length ? (_J$X_("div", { className: cx('Card-toolbar') }, toolbars)) : null;
    };
    CardRenderer.prototype.renderActions = function () {
        ...
        if (Array.isArray(actions)) {
            ...
            return group.map(function (actions, groupIndex) { return (_J$X_("div", { key: groupIndex, className: cx('Card-actions') }, actions.map(function (action, index) {
                ...
                return render("action/".concat(index), tslib.__assign(tslib.__assign({ level: 'link', type: 'button' }, action), { size: size }), {
                    ...
                    className: cx('Card-action', amisCore.filterClassNameObject(action.className || "".concat(size ? "Card-action--".concat(size) : ''), data), {
                        'is-disabled': amisCore.isDisabled(action, data)
                    }),
                    componentClass: 'a',
    ...
    CardRenderer.prototype.renderFeild = function (region, field, key, props) {
        ...
        return (_J$X_("div", { className: cx('Card-field'), key: key },
            useCardLabel && field.label ? (_J$X_("label", { className: cx('Card-fieldLabel', field.labelClassName) }, field.label)) : null,
            render(region, tslib.__assign(tslib.__assign({}, field), { field: field, $$id: $$id, type: 'card-item-field' }), {
                useCardLabel: useCardLabel,
                className: cx('Card-fieldValue', amisCore.filterClassNameObject(field.className, data)),
                rowIndex: itemIndex,
                colIndex: key,
    ...
    CardRenderer.prototype.renderMedia = function () {
        ...
        if (media) {
            ...
            if (type === 'image' && mediaUrl) {
                return (_J$X_("img", { className: cx('Card-multiMedia-img', className), src: mediaUrl }));
            }
            else if (type === 'video' && mediaUrl) {
                return (_J$X_("div", { className: cx('Card-multiMedia-video', className) }, render(region, {
                    type: type,
                    autoPlay: autoPlay,
                    poster: poster,
                    src: mediaUrl,
    ...
    CardRenderer.prototype.render = function () {
        ...
        var headerCn = amisCore.filterClassNameObject(header === null || header === void 0 ? void 0 : header.className, ctx) || headerClassName;
        var titleCn = amisCore.filterClassNameObject(header === null || header === void 0 ? void 0 : header.titleClassName, ctx) || titleClassName;
        var subTitleCn = amisCore.filterClassNameObject(header === null || header === void 0 ? void 0 : header.subTitleClassName, ctx) ||
            subTitleClassName;
        var descCn = amisCore.filterClassNameObject(header === null || header === void 0 ? void 0 : header.descClassName, ctx) || descClassName;
        var descriptionCn = amisCore.filterClassNameObject(header === null || header === void 0 ? void 0 : header.descriptionClassName, ctx) ||
            descriptionClassName ||
            descCn;
        var avatarTextCn = amisCore.filterClassNameObject(header === null || header === void 0 ? void 0 : header.avatarTextClassName, ctx) ||
            avatarTextClassName;
        var avatarCn = amisCore.filterClassNameObject(header === null || header === void 0 ? void 0 : header.avatarClassName, ctx) || avatarClassName;
        var imageCn = amisCore.filterClassNameObject(header === null || header === void 0 ? void 0 : header.imageClassName, ctx) || imageClassName;
        var mediaPosition = media === null || media === void 0 ? void 0 : media.position;
        return (_J$X_(amisUi.Card, tslib.__assign({}, rest, { title: this.rederTitle(), subTitle: this.renderSubTitle(), subTitlePlaceholder: this.renderSubTitlePlaceholder(), description: this.renderDesc(), descriptionPlaceholder: this.renderDescPlaceholder(), children: this.renderBody(), actions: this.renderActions(), avatar: this.renderAvatar(), avatarText: this.renderAvatarText(), secondary: this.renderSecondary(), toolbar: this.renderToolbar(), avatarClassName: avatarCn, avatarTextStyle: this.renderAvatarTextStyle(), avatarTextClassName: avatarTextCn, className: className, titleClassName: titleCn, media: this.renderMedia(), subTitleClassName: subTitleCn, mediaPosition: mediaPosition, descriptionClassName: descriptionCn, imageClassName: imageCn, headerClassName: headerCn, footerClassName: footerClassName, secondaryClassName: secondaryClassName, bodyClassName: bodyClassName, onClick: this.isHaveLink() ? this.handleClick : this.handleCheck })));
    };
