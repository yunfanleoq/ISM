...
var CheckboxesControl = /** @class */ (function (_super) {
    ...
    CheckboxesControl.prototype.renderGroup = function (option, index) {
        ...
        return (_J$X_("div", { key: 'group-' + index, className: cx('CheckboxesControl-group', option.className) },
            _J$X_("label", { className: cx('CheckboxesControl-groupLabel', option.labelClassName) }, option[labelField || 'label']),
            body));
    };
    CheckboxesControl.prototype.renderItem = function (option, index) {
        ...
        return (_J$X_(amisUi.Checkbox, { className: itemClassName, key: index, onChange: function () { return onToggle(option); }, checked: !!~selectedOptions.indexOf(option), disabled: disabled || option.disabled, inline: inline, labelClassName: optionLabelClassName || labelClassName, description: option.description, optionType: optionType },
            menuTpl
                ? render("checkboxes/".concat(index), menuTpl, {
                    data: amisCore.createObject(data, option)
                })
                : labelText,
            removable && amisCore.hasAbility(option, 'removable') ? (_J$X_("a", { "data-tooltip": __('Select.clear'), "data-position": "left" },
                _J$X_(amisUi.Icon, { icon: "minus", className: "icon", onClick: function (e) { return _this.handleDeleteClick(e, option); } }))) : null,
            editable && amisCore.hasAbility(option, 'editable') ? (_J$X_("a", { "data-tooltip": "\u7F16\u8F91", "data-position": "left" },
                _J$X_(amisUi.Icon, { icon: "pencil", className: "icon", onClick: function (e) { return _this.handleEditClick(e, option); } }))) : null));
    };
    ...
    CheckboxesControl.prototype.render = function () {
        ...
        if (checkAll && body.length && optionType === 'default') {
            body.unshift(_J$X_(amisUi.Checkbox, { key: "checkall", className: itemClassName, onChange: onToggleAll, checked: !!selectedOptions.length, partial: inRange__default["default"](selectedOptions.length, 0, amisCore.flattenTreeWithLeafNodes(options).length), disabled: disabled, inline: inline, labelClassName: labelClassName }, checkAllText !== null && checkAllText !== void 0 ? checkAllText : __('Checkboxes.selectAll')));
        }
        body = this.columnsSplit(body);
        return (_J$X_("div", { className: cx("CheckboxesControl", className), ref: "checkboxRef" },
            body && body.length ? (body) : loading ? null : (_J$X_("span", { className: "Form-placeholder" }, __(placeholder))),
            loading ? (_J$X_(amisUi.Spinner, { show: true, icon: "reload", size: "sm", spinnerClassName: cx('Checkboxes-spinner'), loadingConfig: loadingConfig })) : null,
            (creatable || addApi) && !disabled ? (_J$X_("a", { className: cx('Checkboxes-addBtn'), onClick: this.handleAddClick },
