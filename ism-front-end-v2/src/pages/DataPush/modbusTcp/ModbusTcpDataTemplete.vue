    ...
    <a-spin style="padding: 1px;"  :spinning="messageShowLoad" tip="Loading...">
      <a-table rowKey="TempleteName" :pagination="pagination" :columns="columns" :data-source="dataSource">
        ...
        <div slot="action" slot-scope="text, record">
          ...
          <router-link :to="`/DataPush/ModbusTempleteData/${record.TempleteUuid}`"  style="color: darkorange"><a-icon type="import" />{{$t('dataModel.DLT645Model.DataList')}}</router-link> |
    ...
    >
      <a-form :form="PlanForm" layout="vertical" >
        ...
        <a-row :gutter="16">
          <a-col :span="24">
            <a-form-item
                :label="$t('ISMDataTemplete.TempleteDes')"
            >
              <a-input  autocomplete="autocomplete"

                        v-decorator="['TempleteDes', {rules: [{ required: true, message: $t('ISMDataTemplete.TempleteDes'), whitespace: true}]}]"
              />
            </a-form-item>
          </a-col>
        </a-row>
  ...
  data() {
    return {
      ...
      columns: [
        ...
        {
          slotName: 'ISMDataTemplete.TempleteDes',
          width: '40%',
          scopedSlots: {customRender: 'TempleteDes', title: 'ISMDataTemplete.TempleteDes'},
          dataIndex: 'TempleteDes',
        },
        {
          width: '15%',
      ...
      GetTempleteDataList().then(function (res) {
        ...
        if (res.data.code == 200) {
          for(let i=0;i<res.data.list.length;i++){
            if(res.data.list[i].TempleteType==5)
            {
      ...
      this.PlanForm.validateFields((err) => {
        if (!err) {
          const params = {
            ...
            TempleteDes:_t.PlanForm.getFieldValue('TempleteDes'),
            TempleteType:5
          }
