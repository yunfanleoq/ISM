...
export default {
  ...
  data() {
    return {
        ...
        editMode:true,
        base:{
          "text": "configComponent.DeviceStatus.title",
          "icon": "icon-shujulianzhuangtai",
          "isFontIcon": true,
          "info": {
            "type": "chart-gauge",
            "action": [],
            "active": [
              ...
              {
                id:"DeviceOnLineCount",
                name:"SystemData.DeviceOnLineCount",
                result:"",
                isExpression:false,
                condition:{
                  deviceSN:"",
              ...
              {
                id:"DeviceOffLineCount",
                name:"SystemData.DeviceOffLineCount",
                result:"",
                isExpression:false,
                condition:{
                  deviceSN:"",
              ...
              {
                id:"DeviceAlarmCount",
                name:"SystemData.DeviceAlarmCount",
                result:"",
                isExpression:false,
                condition:{
                  deviceSN:"",
            ...
            "style": {
              ...
              "diy":[
                {
                  "name":"configComponent.DeviceStatus.OnLineTextColor",
                  "type":2,
                  "value":"#00ffff",
                  "key":"OnLineTextColor",
                },
                {
                  "name":"configComponent.DeviceStatus.OffLineTextColor",
                  "type":2,
                  "value":'#C0C0C0',
                  "key":"OffLineTextColor",
                },
                {
                  "name":"configComponent.DeviceStatus.AlarmTextColor",
                  "type":2,
                  "value":'#FF4500',
                  "key":"AlarmTextColor",
                },
        ...
        AlarmTextColor:"",
        DeviceCount:100,
        DeviceOnLineCount:40,
        DeviceOffLineCount:30,
        DeviceAlarmCount:10,
        trafficWay:[],
        trafficWayColor:[]
      }
  ...
  methods: {
    optionFunc(){
      let _t = this
      this.trafficWay = [{
        name: this.$t('configComponent.DeviceStatus.OnLineText'),
        value: _t.DeviceOnLineCount*1000
      },{
        name: this.$t('configComponent.DeviceStatus.AlarmText'),
        value: _t.DeviceAlarmCount*1000
      },{
        name: this.$t('configComponent.DeviceStatus.OffLineText'),
        value: _t.DeviceOffLineCount*1000
      }];

      let data = [];
      this.trafficWayColor=[_t.OnLineTextColor,_t.AlarmTextColor,_t.OffLineTextColor]
      ...
      let option = {
        ...
        legend: {
          icon: "circle",
          orient: 'horizontal',
          data:[_t.$t('configComponent.DeviceStatus.OnLineText'),_t.$t('configComponent.DeviceStatus.AlarmText'),_t.$t('configComponent.DeviceStatus.OffLineText')],
          right: '30%',
          bottom: '10%',
          align: 'right',
          textStyle: {
    ...
    initComponents(option){
      ...
      {
        if(option.style.diy[i].key=="OnLineTextColor")
        {
          this.OnLineTextColor = option.style.diy[i].value
        }
        else if(option.style.diy[i].key=="OffLineTextColor")
        {
          this.OffLineTextColor = option.style.diy[i].value
        }
        else if(option.style.diy[i].key=="AlarmTextColor")
        {
          this.AlarmTextColor = option.style.diy[i].value
        }
  ...
  created(){
    ...
    this.$nextTick(function(){
        ...
        _t.$EventBus.$on(activeEvent, _t._activeEventHandler = (data) => {
          let charge = 0
          if(data.ID == "DeviceCount")
          {
            if(_t.DeviceCount!=data.result)
            {
              charge = 1
              _t.DeviceCount = data.result
            }
          }
          else if(data.ID == "DeviceOnLineCount")
          {
            if(_t.DeviceOnLineCount!=data.result) {
              charge = 1
              _t.DeviceOnLineCount = data.result
            }
          }
          else if(data.ID == "DeviceOffLineCount")
          {
            if(_t.DeviceOffLineCount!=data.result) {
              charge = 1
              _t.DeviceOffLineCount = data.result
            }
          }
          else if(data.ID == "DeviceAlarmCount")
          {
            if(_t.DeviceAlarmCount!=data.result) {
              charge = 1
              _t.DeviceAlarmCount = data.result
            }
          }
          if(charge)
          {
