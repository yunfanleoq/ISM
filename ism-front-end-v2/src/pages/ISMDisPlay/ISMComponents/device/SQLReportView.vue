  ...
  <svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="none"  x="0px" y="0px"  xml:space="preserve" :style="{'overflow': 'visible','width':detail.style.position.w,'height':detail.style.position.h,}">
    <g class="svg-el" :class="{'animated':true,[`${detail.style.animate}`]: true}" :style="{'opacity':fillOpacity,'stroke-opacity':strokeOpacity,'stroke':strokeColor,'stroke-width':strokeWidth,'stroke-linecap':'round','stroke-linejoin':'round','fill':fill}">
      <foreignObject style="overflow:visible;" pointer-events="all" :width="detail.style.position.w" :height="detail.style.position.h">
        ...
        <VueOfficeExcel v-else :src="docx" :style="{width:detail.style.position.w+'px',height:detail.style.position.h+'px'}" @rendered="rendered" />
      </foreignObject>
      <!--      闪烁-->
...
import {ExportReportTemplate, ViewReportTemplate} from "@/services/SqlReportTemplete";
import ISMChildAutoMixin from '@/mixins/ISMChildAutoMixin'
export default {
  mixins: [ISMChildAutoMixin],
  name: 'ism-view-sql-xls',
  inject: ['getNode'],
  props: {
  ...
  data() {
    return {
      ...
      docx: '',
      base:{
        "text": "configComponent.viewSQlXls.title",
        "icon": "icon-SQL1",
        "isFontIcon": true,
        "info": {
          "type": "image",
          ...
          "style": {
            ...
            "diy":[
              {
                "name":"configComponent.viewSQlXls.doc",
                "type":32,
                "value":"",
                "key":"SqlUrl",
              },
              {
                "name":"configComponent.viewSQlXls.RefreshTimer",
                "type":1,
                "value":10,
                "key":"RefreshTimer",
              },
  ...
  methods: {
    initComponents(option){
      ...
      {
        if(option.style.diy[i].key=="SqlUrl")
        {
          if(option.style.diy[i].value!="") {
            this.SQLViewUUID = option.style.diy[i].value
            this.ViewSQLReport()
        ...
        else if(option.style.diy[i].key=="RefreshTimer")
        {
          this.TimeInterval = parseInt(option.style.diy[i].value)*60*1000
    ...
    ViewSQLReport(rc){
      let _t = this
      let params = {
        uuid:this.SQLViewUUID
      };
      ViewReportTemplate(params).then(function (res){
        if(res.data.code==0)
        {
          _t.docx = "/" + res.data.path
          setTimeout(function () {
            _t.isShowDocx = true
          }, 100)
        }else{
