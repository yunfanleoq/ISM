          ...
          <a-row>
            ...
            <a-col :md="9" :sm="24">
              ...
              >
                <a-radio-group v-model="SelectDateType" @change="chargeDateType">
                  <a-radio-button value="Day">{{ $t('reporting.AlarmHistory.DateDayType') }}</a-radio-button>
                  <a-radio-button value="Weekly">{{ $t('reporting.AlarmHistory.DateWeeklyType') }}</a-radio-button>
                  <a-radio-button value="Month">{{ $t('reporting.AlarmHistory.DateMonthType') }}</a-radio-button>
                  <a-radio-button value="Year">{{ $t('reporting.AlarmHistory.DateYearType') }}</a-radio-button>
                </a-radio-group>
              </a-form-item>
            </a-col>
            ...
            <a-col :md="3" :sm="24">
              <span style="display: inline-block; margin-top: 3px;">
                <a-button :disabled="messageShowLoad" type="primary" @click="QueryHistoryDataDifferent">
...
export default {
  ...
  name: 'ism-view-history-difference',
  inject: ['getNode'],
  props: {},
  ...
  computed: {
    ...
    pagedData() {
      ...
      const pageRows = this.dynamicData.slice(start, end);
      return this.isLastPage ? pageRows.concat(this.summaryRows) : pageRows;
    },
    isLastPage() {
      const pageSize = this.pagination && this.pagination.pageSize ? this.pagination.pageSize : 10;
      const maxPage = Math.max(1, Math.ceil(this.dynamicData.length / pageSize));
      const current = this.pagination && this.pagination.current ? this.pagination.current : 1;
      return this.dynamicData.length > 0 && current >= maxPage;
    },
    paginationTotal() {
    ...
    dynamicColumns() {
      const columns = [
        ...
        },
        {
          ...
          customRender: (text, record) => (record.isSummaryRow ? '----' : this.getAlias(record.rowName))
        },
        {
          title: '数据名称',
          key: 'realName',
          ...
          customRender: (text, record) => (record.isSummaryRow ? record.summaryName : this.getRealName(record.rowName))
        }
      ];
  ...
  data() {
    return {
      ...
      foreColor: '',
      base: {
        text: 'configComponent.viewHistroyDifference.title',
        icon: 'icon-biaozhunchazhi',
        isFontIcon: true,
        info: {
          type: 'image',
          action: [],
  ...
  methods: {
    ...
    chargeDateType(e) {
      ...
      if (type === 'Day') {
        ...
        this.columnNames = this.generateHourColumns();
      } else if (type === 'Weekly') {
        const startDate = moment().day(1).format('YYYY-MM-DD');
        const endDate = moment().day(7).format('YYYY-MM-DD');
        this.SelectDateRange = [startDate, endDate];
        this.columnNames = this.generateWeekColumns();
      } else if (type === 'Month') {
        this.SelectDateRange = moment().format('YYYY-MM');
        this.columnNames = this.generateMonthColumns(this.SelectDateRange);
      } else if (type === 'Year') {
        this.SelectDateRange = moment().format('YYYY');
        this.columnNames = this.generateYearColumns();
      } else {
        this.SelectDateRange = moment().format('YYYY-MM-DD');
        this.columnNames = this.generateHourColumns();
      }
    },
    onWeeklyDateChange(date) {
      if (!date) return;
    ...
    QueryHistoryDataDifferent() {
      if (this.SelectDateType === 'Day') {
        this.QueryHistoryDayDifferent();
      } else if (this.SelectDateType === 'Weekly') {
        this.QueryHistoryWeeklyDifferent();
      } else if (this.SelectDateType === 'Month') {
        this.QueryHistoryMonthDifferent();
      } else if (this.SelectDateType === 'Year') {
        this.QueryHistoryYearDifferent();
      }
    },
    buildRequestParams() {
      return {
    ...
    QueryHistoryDayDifferent() {
      this.columnNames = this.generateHourColumns();
      this.messageShowLoad = true;
      GetHistoryDayDifference(this.buildRequestParams())
        .then(res => {
          if (res.data.code === 0) {
            this.batchUpdateConfig(res.data.realData);
            this.updateExportData('日报表_', this.SelectDateRange);
          }
    ...
    QueryHistoryWeeklyDifferent() {
      this.columnNames = this.generateWeekColumns();
      this.messageShowLoad = true;
      GetHistoryWeeklyDifference(this.buildRequestParams())
        .then(res => {
          if (res.data.code === 0) {
            this.batchUpdateConfig(res.data.realData);
            const dateRangeStr = Array.isArray(this.SelectDateRange) ? this.SelectDateRange.join('_') : this.SelectDateRange;
            this.updateExportData('周报表_', dateRangeStr);
          }
          this.messageShowLoad = false;
        })
        .catch(() => {
    ...
    QueryHistoryMonthDifferent() {
      this.columnNames = this.generateMonthColumns(this.SelectDateRange);
      this.messageShowLoad = true;
      GetHistoryMonthDifference(this.buildRequestParams())
        .then(res => {
          if (res.data.code === 0) {
            this.batchUpdateConfig(res.data.realData);
            this.updateExportData('月报表_', this.SelectDateRange);
          }
          this.messageShowLoad = false;
        })
        .catch(() => {
    ...
    QueryHistoryYearDifferent() {
      this.columnNames = this.generateYearColumns();
      this.messageShowLoad = true;
      GetHistoryYearDifference(this.buildRequestParams())
        .then(res => {
          if (res.data.code === 0) {
            this.batchUpdateConfig(res.data.realData);
            this.updateExportData('年报表_', this.SelectDateRange);
          }
          this.messageShowLoad = false;
        })
        .catch(() => {
