...
import {login, getRoutesConfig, logout} from '@/services/user'
import {mapState, mapMutations} from 'vuex'
import Cookie from 'js-cookie'
import ISMChildAutoMixin from '@/mixins/ISMChildAutoMixin'
export default {
  mixins: [ISMChildAutoMixin],
  name: 'view-svg-login-out-button',
  inject: ['getNode'],
  ...
  data() {
    return {
      ...
      textY:0,
      base:{
        "text": "displayConfig.ToolBox.login.loginOutBtn",
        "icon": "icon-dengchu",
        "isFontIcon": true,
        "info": {
          "type": "image",
  ...
  methods: {
    ...
    loginRequest(){
      ...
      if(this.detail.action.length>0)
      {
        setTimeout(function (){
          logout()
          location.reload()
        },1000)
        if(this.detail.action[0].linkType=="External")
        {
          if(typeof this.detail.action[0].OpenExternalType=='undefined') {
            window.open(this.detail.action[0].External, '_blank')
          }else if( this.detail.action[0].OpenExternalType=='self'){
            window.open(this.detail.action[0].External, '_self')
          }else if(this.detail.action[0].OpenExternalType=='new'){
            window.open(this.detail.action[0].External, '_blank')
          }
        }
      }
  ...
  created(){
    ...
    _t.$EventBus.$on('cell-vuex', (data) => {
      ...
      _t.PMapMutations = data.PMapMutations
      _t.initComponents(_t.detail)
    })
  }
}
