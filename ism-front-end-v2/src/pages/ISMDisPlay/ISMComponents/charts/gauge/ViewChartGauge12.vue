...
export default {
  ...
  data() {
    return {
      ...
      editMode:true,
        base:{
          "text": "configComponent.PercentChart1.title",
          "icon": "icon-fsux_tubiao_baifenbihuantu",
          "isFontIcon": true,
          "info": {
            "type": "chart-gauge",
            "action": [],
            ...
            "style": {
              ...
              "diy":[
                ...
                {
                  "name":"configComponent.PercentChart1.progressColor",
                  "type":2,
                  "value":"#60F4C2",
                  "key":"progressColor",
                },
                {
                  "name":"configComponent.PercentChart1.surplusColor",
                  "type":2,
                  "value":"#0D0D0D",
                  "key":"surplusColor",
                },
  ...
  methods: {
    initComponents(option){
      ...
      this.option.title[0].text = this.option.series[0].data[0]+ this.chartUnit
      this.option.title[0].textStyle.color = this.detail.style.foreColor
      this.option.title[0].textStyle.fontSize = this.detail.style.fontSize
      this.option.title[0].textStyle.fontFamily = this.detail.style.fontFamily
      this.option.series[0].backgroundColor = this.detail.style.backColor
  ...
  created(){
    ...
    node.on('change:visible', ({ current }) => {
      if(current)
      {
        const temp = _t.option
        _t.initComponents(_t.detail);
        this.echartsView.setOption(temp,true)
      }
    });
    ...
    this.$nextTick(function(){
        ...
        _t.$EventBus.$on(activeEvent, (data) => {
          if(data.ID == "ShowData")
          {
            ...
            {
              _t.option.series[0].data[0] = data.result
              _t.option.title[0].text = _t.option.series[0].data[0]+ _t.chartUnit
