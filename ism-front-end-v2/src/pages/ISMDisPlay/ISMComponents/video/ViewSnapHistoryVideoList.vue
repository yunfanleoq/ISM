<template>
  <svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="none"  x="0px" y="0px"  xml:space="preserve" :style="{'overflow': 'visible','width':detail.style.position.w,'height':detail.style.position.h,}">
    ...
    <div class="h5videodiv" :style="{'width': width+'px','height': height+'px',}" >
      ...
      <div  class="video-close channel-selector" @click="showSystemVideoModel()" >选择抓拍视频</div>
    </div>
          <system-snap-history-video-model @onSelectVideo="onSelectVideo" :networkVideo="videoComponentData" ref="systemVideoModel"></system-snap-history-video-model>
</div>
        </div>
      </foreignObject>
      <!--      闪烁-->
...
export default {
  ...
  name: 'ism-view-snap-history-video-list',
  inject: ['getNode'],
  ...
  data() {
    return {
      ...
      height:600,
      base:{
        "text": "configComponent.video.SnapHistoryVideoList",
        "icon": "icon-CCTV",
        "isFontIcon": true,
        "info": {
          "type": "image",
