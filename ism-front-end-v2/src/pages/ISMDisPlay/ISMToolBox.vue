            ...
            <div class="toolbox-group scrollbar">
              <template v-for="(value,index) in validResultsList(group.items)">
                <div
                    ...
                    @mouseenter="showTooltip($event, group.isSequence ? $t(group.title)+(index+1) : $t(value.text))"
        ...
        <div class="toolbox-group scrollbar" v-if="(Array.isArray(ShowMoreItemArray.items))&&(ShowMoreItemArray.items.length>0)">
          <template v-for="(value,index) in MoreItemList(ShowMoreItemArray.items)" >
            <div
                ...
                draggable="true"
                @dragstart="onDragstart($event,value)"
                @mouseenter="showTooltip($event, ShowMoreItemArray.isSequence ? $t(ShowMoreItemArray.title)+(index+21) : $t(value.text))"
                @mouseleave="hideTooltip"
            >
              <template >
                <div class="toolbox-item-icon">
...
export default {
  ...
  computed: {
    ...mapState({
      toolbox: state => store.state.ISMDisPlayEditorTool.toolBoxList,
    }),
    validResultsList(){
      return item => {
