...
export default {
  ...
  methods: {
    ...
    pageDisplayTitle(item){
      ...
      // 默认隐藏「运行模板/旧版」后缀；调试时 localStorage.ismShowPageTemplateTags=1 或 ?showPageTags=1
      const showTags = (typeof localStorage !== 'undefined' && localStorage.getItem('ismShowPageTemplateTags') === '1')
        || (this.$route && this.$route.query && String(this.$route.query.showPageTags) === '1')
      if (!showTags) {
        return item.title
      }
        ...
        if (!err) {
          ...
          DisplayModelPageAdd(params).then(function (res){
            ...
            if (code == 4002) {
              ...
              _t.setEditorRuntimePreview(null)
              _t.setNavContext(null)
              _t.getLayerDataStruct({uuid:_t.$route.params.uid,metaOnly:true,cb:function (){
                  _t.spinning = false
                  _t.logging = false
                  // 新建后自动切到新页，避免仍保存到旧 pageUuid
                  if (newPageId) {
                    // 强制写入选中页，防止 meta 刷新后仍指向旧页
                    try {
            ...
            _t.logging = false
            _t.$message.error((_t.$t('loginPage.serverError') || '服务异常') + (err && err.message ? `: ${err.message}` : ''), 3)
          })
        }
      })
    },
