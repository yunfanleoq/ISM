<template>
  <a-card>
      <a-space class="operator">
        <a-button @click="dnp3Add()" type="primary" icon="plus">{{$t('dataModel.newModel')}}</a-button>
    ...
    <a-spin style="padding: 1px;"  :spinning="messageShowLoad" tip="Loading...">
      ...
      <div slot="action" slot-scope="text, record">
        <router-link :to="`/DeviceModel/DNP3ModelDetail/${record.key}`" style="color: #13C2C2"><a-icon type="edit" />{{$t('dataModel.modelDetail')}}</router-link> |
        <router-link :to="`/DeviceModel/DNP3ModelData/${record.key}`"  style="color: darkorange"><a-icon type="import" />{{$t('dataModel.DLT645Model.DataList')}}</router-link> |
...
<script>
import {DNP3ModelList, DNP3ModelDel} from "../../../services/dnp3Model";
export default {
  name: 'DNP3ModelList',
  ...
  methods: {
    ...
    getModelList(){
      ...
      DNP3ModelList(params).then(function (res){
        let tableData={}
    ...
    deleteRecord(key) {
      ...
      let _t = this
      DNP3ModelDel(params).then(function (res) {
    ...
    dnp3Add(){
      this.$router.push('/DeviceModel/DNP3ModelAdd')
    },
  }
}
</script>
