  ...
  <a-card>
    ...
    <a-spin style="padding: 1px;"  :spinning="messageShowLoad" tip="Loading...">
      ...
      <div slot="action" slot-scope="text, record">
        <router-link :to="`/DeviceModel/IEC61850Detail/${record.key}`" style="color: #13C2C2"><a-icon type="edit" />{{$t('dataModel.modelDetail')}}</router-link> |
        <router-link :to="`/DeviceModel/IEC61850Nodeid/${record.key}`"  style="color: darkorange"><a-icon type="unordered-list" />{{$t('dataModel.opcuaModel.NodeIDConfig')}}</router-link> |
...
<script>
import {IEC61850Modellist,IEC61850ModelDelete} from "@/services/IEC61850Model";
export default {
  name: 'IEC61850ModelList',
  ...
  methods: {
    ...
    getModelList(){
      ...
      IEC61850Modellist(params).then(function (res){
        let tableData={}
    ...
    deleteRecord(key) {
      ...
      let _t = this
      IEC61850ModelDelete(params).then(function (res) {
    ...
    opcuaAdd(){
      this.$router.push('/DeviceModel/IEC61850Add')
    },
  }
}
</script>
