    ...
    <a-card  style="min-height: 400px">
      <a-space class="operator">
        ...
        <a-button type="primary" @click="RegisterVisible=true;isEdit=false"> <a-icon type="plus" />
          {{$t("dataModel.opcuaModel.AddNodeID")}}</a-button>
        ...
        >
          <a-button  type="primary"> <a-icon type="upload" />
            {{$t("dataModel.opcuaModel.ImportNodeID")}}</a-button>
        </a-upload>
        ...
        <a-button type="link">
          <a href="https://www.ismctl.com/soft/IEDExplorer_0.80a.zip">{{$t("dataModel.opcuaModel.ImportTool")}}</a>
        </a-button>
      ...
      <a-spin style="padding: 1px;"  :spinning="messageShowLoad" tip="Loading...">
        <a-table  :pagination="pagination" rowKey="name" :columns="registerGroupColumns" :data-source="registerGroupDataSource" class="ant-table-tbody">
          ...
          <template slot="NodeIDDataType" slot-scope="text">
            ...
            <span v-else-if="text==4"> VisibleString</span>
        ...
        <a-spin style="padding: 1px;"  :spinning="ShowRegisterLoading" tip="Loading...">
            <a-form :form="RegisterForm" layout="vertical" @submit="AddNodeId">
              ...
              >
                ...
                <a-col :span="12">
                  <a-form-item
                  :label="$t('dataModel.opcuaModel.NodeIDPath')"
              >
                <a-input autocomplete="autocomplete"
                         v-decorator="['NodeIDPath', {rules: [{ required: true, message: $t('dataModel.opcuaModel.NodeIDPath'), whitespace: true}]}]"
                />
              </a-form-item>
                </a-col>
              </a-row>
              <a-row :gutter="16">
                ...
                >
                  ...
                  >
                    ...
                    <a-select-option value="4">VisibleString</a-select-option>
                ...
                <a-col :span="12">
                  <a-form-item
                      :label="$t('dataModel.opcuaModel.NodeIDDataFunType')"
                  >
                    <a-select class="DataType" autocomplete="autocomplete"
                              v-decorator="['NodeIDDataFunType', {rules: [{ required: true, message: $t('dataModel.opcuaModel.NodeIDDataFunType'), whitespace: true}]}]"
                    >
                      <a-select-option value="1">ST</a-select-option>
                      <a-select-option value="2">MX</a-select-option>
                      <a-select-option value="3">SP</a-select-option>
                      <a-select-option value="4">SV</a-select-option>
                      <a-select-option value="5">CF</a-select-option>
                      <a-select-option value="6">DC</a-select-option>
                      <a-select-option value="7">SG</a-select-option>
                      <a-select-option value="8">SE</a-select-option>
                      <a-select-option value="9">SR</a-select-option>
                      <a-select-option value="10">OR</a-select-option>
                      <a-select-option value="11">BL</a-select-option>
                      <a-select-option value="12">EX</a-select-option>
                      <a-select-option value="13">CO</a-select-option>
                      <a-select-option value="14">US</a-select-option>
                      <a-select-option value="15">MS</a-select-option>
                      <a-select-option value="16">RP</a-select-option>
                      <a-select-option value="17">BR</a-select-option>
                      <a-select-option value="18">LG</a-select-option>
                      <a-select-option value="19">GO</a-select-option>
                    </a-select>
                  </a-form-item>
                </a-col>
                <a-col :span="12">
...
} from "@/services/IEC61850Model";
import {IEC61850ImportNodeID, LOCALUPGATEDATAMODEL} from "@/services/api";
import { exportExcelWithStyle } from "@/services/excelExport.js"
const dataSource= []
const loadingKey = 'updatable'
export default {
  name: 'IEC61850ModelNodeID',
  ...
  data () {
    return {
      ...
      exportName:"IEC61850数据模板",
      localUpgradeUrl:LOCALUPGATEDATAMODEL+"/"+this.$route.params.uid,
      searchText: '',
      searchInput: null,
      searchedColumn: '',
      DataRecordType:0,
      importUrl:IEC61850ImportNodeID+"/"+this.$route.params.uid,
      ...
      registerGroupColumns: [
        ...
        {
          slotName:this.$t("dataModel.opcuaModel.NodeIDPath"),
          scopedSlots: {  customRender: 'NodeIDPath'  ,title:this.$t("dataModel.opcuaModel.NodeIDPath")},
          width: '25%',
          align:"left",
          dataIndex: 'NodeIDPath'
        },
        {
  ...
  methods: {
        ...
        _t.RegisterForm.setFieldsValue(
            {
              ...
              NodeIDDataFunType:item.FunType,
        ...
        if (!err) {
          let params = {
            ...
            data: {
              ...
              FunType:this.RegisterForm.getFieldValue('NodeIDDataFunType'),
          ...
          let _t = this
          IEC61850ModelNodeIDEdit(params).then(function (res){
    ...
    NodeIdList(){
      ...
      IEC61850ModelNodeIDList(params).then(function (res){
    ...
    AddNodeId(){
      this.RegisterForm.validateFields((err) => {
        if (!err) {
          ...
          this.RegisterVisible = false;
          let _t = this
          IEC61850ModelNodeIDAdd(params).then(function (res){
            _t.messageShowLoad = false
            if(res.data.code==2002)
            {
              _t.NodeIdList()
    ...
    deleteRecord(uuid,muid) {
      ...
      IEC61850ModelNodeIDDel(params).then(function (res) {
    ...
    onBlackCLK(){
      this.$router.push('/DeviceModel/IEC61850Model')
    },
    alarmCharge(value){
