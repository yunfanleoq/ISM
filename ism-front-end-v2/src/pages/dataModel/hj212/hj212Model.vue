<template>
  <a-card>
      <a-space class="operator">
        <a-button @click="HJ212Add()" type="primary" icon="plus">{{$t('dataModel.newModel')}}</a-button>
    ...
    <a-spin style="padding: 1px;"  :spinning="messageShowLoad" tip="Loading...">
      ...
      <div slot="action" slot-scope="text, record">
        <router-link :to="`/DeviceModel/HJ212Detail/${record.key}`" style="color: #13C2C2"><a-icon type="edit" />{{$t('dataModel.modelDetail')}}</router-link> |
        <router-link :to="`/DeviceModel/HJ212Nodeid/${record.key}`"  style="color: darkorange"><a-icon type="unordered-list" />{{$t('dataModel.opcuaModel.NodeIDConfig')}}</router-link> |
...
<script>
import {HJ212ModelList,HJ212ModelDelete} from "@/services/hj212Model";
export default {
  name: 'HJ212ModelList',
  ...
  methods: {
    ...
    getModelList(){
      ...
      HJ212ModelList(params).then(function (res){
        let tableData={}
    ...
    deleteRecord(key) {
      ...
      let _t = this
      HJ212ModelDelete(params).then(function (res) {
    ...
    HJ212Add(){
      this.$router.push('/DeviceModel/HJ212Add')
    },
  }
}
</script>
