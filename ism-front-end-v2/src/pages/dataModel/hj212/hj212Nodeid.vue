...
<!--    添加节点-->
    ...
    <a-drawer
          :title="isEdit?$t('dataModel.HJ212Model.EditNodeID'):$t('dataModel.HJ212Model.AddNodeID')"
          :width="720"
                ...
                <a-col :span="12">
                  ...
                  >
                    <a-select
                        ...
                        @search="handleSelectSearch" allowClear show-search optionFilterProp="children" style="width: 100%" :token-separators="[',']">
                      <a-select-option v-for="(alarmItem,itemIndex) in frontDataZ" :key="itemIndex" :value=alarmItem.name>
                        {{ $t(alarmItem.name) }}
                      </a-select-option>
                    </a-select>
                  </a-form-item>
...
import {
  ...
  HJ212ModelNodeIDList, HJ212TempleteIDList
} from "@/services/hj212Model";
...
export default {
  ...
  data () {
    return {
      ...
      registerGroupColumns: [
        ...
        {
          slotName:this.$t("dataModel.HJ212Model.EncodeID"),
          scopedSlots: {  customRender: 'EncodeID'  ,title:this.$t("dataModel.HJ212Model.EncodeID")},
          width: '15%',
          align:"left",
          dataIndex: 'EncodeID'
        },
  ...
  methods: {
        ...
        if (!err) {
          let params = {
            ...
            data: {
              ...
              EncodeID: this.RegisterForm.getFieldValue('Identifier'),
          ...
          let _t = this
         HJ212ModelNodeIDEdit(params).then(function (res){
    ...
    NodeIdList(){
     ...
     HJ212ModelNodeIDList(params).then(function (res){
    ...
    SelectTempleteData(e){
      let _t = this
      this.TempleteDataList.forEach(item => {
        if (item.name.indexOf(e) >= 0) {
          console.log(item)
          _t.RegisterForm.setFieldsValue(
              {
                Identifier:item.code,
                NodeIDDataType:item.type=="float"?"3":"1",
                dataUnit:item.unit,
              })
        }
      })
    ...
    handlePopupScroll (e) {
      ...
      if (scrollHeight === 0 && clientHeight === 0) {
        ...
        if (scrollHeight < clientHeight + 5) {
          ...
          if (this.TempleteDataList.length > treePageSize) {
            // 如果总数据的条数大于需要展示的数据
            max = treePageSize
          } else {
            // 否则
            max = this.TempleteDataList.length
          }
          // 判断是否有搜索
          if (this.valueData) {
            ...
            this.TempleteDataList.forEach((item, index) => {
              if (index < max) { // 当data数组的下标小于max时
                newData.push(item)
              }
            })
    ...
    TempleteIdList(){
      ...
      let _t = this
      _t.TempleteDataList = []
      HJ212TempleteIDList().then(function (res){
        _t.messageShowLoad = false
        if(res.data.code==0)
        {
          _t.TempleteDataList = res.data.list
          _t.frontDataZ = _t.TempleteDataList.slice(0, 100)
        }
      }).catch(function (){
        _t.messageShowLoad = false
        _t.$message.error(_t.$t('loginPage.serverError'), 3)
    ...
    AddNodeId(){
      this.RegisterForm.validateFields((err) => {
        if (!err) {
          ...
          this.RegisterVisible = false;
          let _t = this
          HJ212ModelNodeIDAdd(params).then(function (res){
            _t.messageShowLoad = false
            if(res.data.code==2002)
            {
              _t.NodeIdList()
    ...
    deleteRecord(uuid,muid) {
      ...
      HJ212ModelNodeIDDel(params).then(function (res) {
