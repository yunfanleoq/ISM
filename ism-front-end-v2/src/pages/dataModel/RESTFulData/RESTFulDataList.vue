...
} from "@/services/RESTFulModel";
const dataSource= []
const loadingKey = 'updatable'
export default {
  name: 'RESTFulDataList',
  ...
  data () {
    return {
      ...
      registerGroupColumns: [
        ...
        {
          slotName:this.$t("dataModel.RESTFulData.NodeIDPath"),
          scopedSlots: {  customRender: 'uuid'  ,title:this.$t("dataModel.RESTFulData.NodeIDPath")},
          width: '20%',
          align:"left",
          dataIndex: 'uuid'
        },
  ...
  methods: {
    ...
    save() {
      this.RegisterForm.validateFields((err) => {
        if (!err) {
          ...
          let _t = this
          RESTFulModelDataEdit(params).then(function (res){
    ...
    RESTFulDataList(){
      ...
      RESTFulModelDataList(params).then(function (res){
    ...
    AddNodeId(){
      this.RegisterForm.validateFields((err) => {
        if (!err) {
          ...
          this.RegisterVisible = false;
          let _t = this
          RESTFulModelDataAdd(params).then(function (res){
            _t.messageShowLoad = false
            if(res.data.code==2002)
            {
              _t.RESTFulDataList()
    ...
    deleteRecord(uuid,muid) {
      ...
      RESTFulModelDataDel(params).then(function (res) {
    ...
    onBlackCLK(){
      this.$router.push('/DeviceModel/RestFulModel')
    },
    alarmCharge(value){
