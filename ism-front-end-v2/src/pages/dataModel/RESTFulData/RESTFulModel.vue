    ...
    <a-spin style="padding: 1px;"  :spinning="messageShowLoad" tip="Loading...">
      <a-table rowKey="key" :pagination="pagination" :columns="columns" :data-source="dataSource">
        ...
        <div slot="action" slot-scope="text, record">
          ...
          <router-link :to="`/DeviceModel/RestFulData/${record.key}`"  style="color: darkorange"><a-icon type="unordered-list" />{{$t('dataModel.opcuaModel.NodeIDConfig')}}</router-link> |
...
<script>
import {OpcuaModellist, OpcuaModelDelete, OpcuaModelAdd, OpcuaModelEdit} from "@/services/opcuaModel";
import Mtextarea from "@/components/textarea";
import {CustomDataEdit} from "@/services/customDataModel";
import {displayModelList, getDisplayModelLayerData} from "@/services/displayModel";
import {RESTFulModelAdd, RESTFulModelDelete, RESTFulModelEdit, RESTFulModellist} from "@/services/RESTFulModel";
export default {
  name: 'RESTFulModel',
  ...
  methods: {
      ...
      if (this.IsEdit)
      {
        this.RESTFulForm.validateFields((err) => {
          if (!err) {
            ...
            RESTFulModelEdit(params).then(function (res){
      ...
      else
      {
        this.RESTFulForm.validateFields((err) => {
          if (!err) {
            ...
            RESTFulModelAdd(params).then(function (res){
              _t.messageShowLoad = false
    ...
    getModelList(){
      ...
      this.messageShowLoad=true
      RESTFulModellist(params).then(function (res){
        let tableData={}
    ...
    deleteRecord(key) {
      ...
      let _t = this
      RESTFulModelDelete(params).then(function (res) {
        if(res.data.code==200)
        {
