<template>
  <a-card>
      <a-space class="operator">
        <a-button @click="BacnetAdd()" type="primary" icon="plus">{{$t('dataModel.newModel')}}</a-button>
    ...
    <a-spin style="padding: 1px;"  :spinning="messageShowLoad" tip="Loading...">
      ...
      <div slot="action" slot-scope="text, record">
        <router-link :to="`/DeviceModel/BACnetDetail/${record.key}`" style="color: #13C2C2"><a-icon type="edit" />{{$t('dataModel.modelDetail')}}</router-link> |
        <router-link :to="`/DeviceModel/BACnetNodeid/${record.key}`"  style="color: darkorange"><a-icon type="unordered-list" />{{$t('dataModel.opcuaModel.NodeIDConfig')}}</router-link> |
...
<script>
import {  BACnetModelList, BACnetModelDelete} from "@/services/bacnetModel";
export default {
  name: 'MQTTModelList',
  ...
  methods: {
    ...
    getModelList(){
      ...
      BACnetModelList(params).then(function (res){
        let tableData={}
    ...
    deleteRecord(key) {
      ...
      let _t = this
      BACnetModelDelete(params).then(function (res) {
    ...
    BacnetAdd(){
      this.$router.push('/DeviceModel/BacnetAdd')
    },
  }
}
</script>
