    ...
    <a-card  style="min-height: 400px">
      ...
      <a-spin style="padding: 1px;"  :spinning="messageShowLoad" tip="Loading...">
        <a-table  :pagination="pagination" rowKey="name" :columns="registerGroupColumns" :data-source="registerGroupDataSource" class="ant-table-tbody">
          ...
          <template slot="BACnetZone" slot-scope="text">
            <span v-if="text==0">{{$t('dataModel.BACnet.BACnetZoneAI')}}</span>
            <span v-if="text==1">{{$t('dataModel.BACnet.BACnetZoneAO')}}</span>
            <span v-if="text==2">{{$t('dataModel.BACnet.BACnetZoneAV')}}</span>
            <span v-if="text==3">{{$t('dataModel.BACnet.BACnetZoneBI')}}</span>
            <span v-if="text==4">{{$t('dataModel.BACnet.BACnetZoneBO')}}</span>
            <span v-if="text==5">{{$t('dataModel.BACnet.BACnetZoneBV')}}</span>
            <span v-if="text==6">{{$t('dataModel.BACnet.BACnetZoneMSI')}}</span>
            <span v-if="text==7">{{$t('dataModel.BACnet.BACnetZoneMSO')}}</span>
            <span v-if="text==8">{{$t('dataModel.BACnet.BACnetZoneMSV')}}</span>
            <span v-if="text==9">{{$t('dataModel.BACnet.BACnetZoneDEV')}}</span>
            <span v-if="text==10">{{$t('dataModel.BACnet.BACnetZoneACC')}}</span>
          </template>
          <template slot="NodeIDDataType" slot-scope="text">
            <span v-if="text==1"> Bit</span>
        ...
        <a-spin style="padding: 1px;"  :spinning="ShowRegisterLoading" tip="Loading...">
            <a-form :form="RegisterForm" layout="vertical" @submit="AddNodeId">
              ...
              >
                ...
                <a-col :span="12">
                  <a-form-item :label="$t('dataModel.BACnet.BACnetZone')">
                    <a-select  @change="BACnetZoneCharge"   autocomplete="autocomplete"  v-decorator="[
                          'BACnetZone',
                          {
                            rules: [{ required: true, message: $t('dataModel.editData.dataAlarm') }],
                          },
                        ]">
                      <a-select-option value="0">{{$t('dataModel.BACnet.BACnetZoneAI')}}</a-select-option>
                      <a-select-option value="1">{{$t('dataModel.BACnet.BACnetZoneAO')}}</a-select-option>
                      <a-select-option value="2">{{$t('dataModel.BACnet.BACnetZoneAV')}}</a-select-option>
                      <a-select-option value="3">{{$t('dataModel.BACnet.BACnetZoneBI')}}</a-select-option>
                      <a-select-option value="4">{{$t('dataModel.BACnet.BACnetZoneBO')}}</a-select-option>
                      <a-select-option value="5">{{$t('dataModel.BACnet.BACnetZoneBV')}}</a-select-option>
                      <a-select-option value="6">{{$t('dataModel.BACnet.BACnetZoneMSI')}}</a-select-option>
                      <a-select-option value="7">{{$t('dataModel.BACnet.BACnetZoneMSO')}}</a-select-option>
                      <a-select-option value="8">{{$t('dataModel.BACnet.BACnetZoneMSV')}}</a-select-option>
                      <a-select-option value="9">{{$t('dataModel.BACnet.BACnetZoneDEV')}}</a-select-option>
                      <a-select-option value="10">{{$t('dataModel.BACnet.BACnetZoneACC')}}</a-select-option>
                    </a-select>
                  </a-form-item>
                </a-col>
                <a-col :span="12">
                  <a-form-item
                      :label="$t('dataModel.BACnet.BACnetAddress')"
                  >
                    <a-input autocomplete="autocomplete"
                             v-decorator="['BACnetAddress', {rules: [{ required: true, message: $t('dataModel.BACnet.BACnetAddress'), whitespace: true}]}]"
                    />
                  </a-form-item>
                </a-col>
                <a-col :span="12">
                ...
                >
                  ...
                  >
                    <a-select-option value="1">Bit</a-select-option>
...
export default {
  ...
  data () {
    return {
      ...
      registerGroupColumns: [
        ...
        {
          slotName:this.$t("dataModel.BACnet.BACnetZone"),
          scopedSlots: {  customRender: 'BACnetZone'  ,title:this.$t("dataModel.BACnet.BACnetZone")},
          width: '10%',
          align:"left",
          dataIndex: 'BacnetZone'
        },
        {
          slotName:this.$t("dataModel.BACnet.BACnetAddress"),
          scopedSlots: {  customRender: 'BACnetAddress'  ,title:this.$t("dataModel.BACnet.BACnetAddress")},
          width: '8%',
          align:"left",
          dataIndex: 'BacnetAddress'
        },
        {
  ...
  methods: {
    ...
    edit(item) {
      ...
      this.BACnetZone = item.BacnetZone
        ...
        _t.RegisterForm.setFieldsValue(
            {
              ...
              BACnetZone:_t.BACnetZone.toString(),
              BACnetAddress:item.BacnetAddress.toString(),
        ...
        if (!err) {
          let params = {
            ...
            data: {
              ...
              BacnetZone:this.BACnetZone,
              BacnetAddress:parseInt(this.RegisterForm.getFieldValue('BACnetAddress')),
          ...
          let _t = this
          BACnetModelNodeIDEdit(params).then(function (res){
            if(res.data.code==2002)
            {
              ...
              if (target) {
                ...
                target.BacnetZone = _t.BACnetZone
                target.BacnetAddress = _t.RegisterForm.getFieldValue('BACnetAddress')
    ...
    NodeIdList(){
      ...
      BACnetModelNodeIDList(params).then(function (res){
    ...
    AddNodeId(){
      this.RegisterForm.validateFields((err) => {
        if (!err) {
          ...
          this.RegisterVisible = false;
          let _t = this
          BACnetModelNodeIDAdd(params).then(function (res){
            _t.messageShowLoad = false
            if(res.data.code==2002)
            {
              _t.NodeIdList()
    ...
    deleteRecord(uuid,muid) {
      ...
      BACnetModelNodeIDDel(params).then(function (res) {
    ...
    onBlackCLK(){
      this.$router.push('/DeviceModel/BACnetModel')
    },
    alarmCharge(value){
    ...
    BACnetZoneCharge(value){
      this.BACnetZone=parseInt(value)
    },
    recordCharge(value){
