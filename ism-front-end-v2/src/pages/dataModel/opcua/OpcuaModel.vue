<template>
  <a-card>
      <a-space class="operator">
        <a-button @click="opcuaAdd()" type="primary" icon="plus">{{$t('dataModel.newModel')}}</a-button>
    ...
    <a-spin style="padding: 1px;"  :spinning="messageShowLoad" tip="Loading...">
      ...
      <div slot="action" slot-scope="text, record">
        <router-link :to="`/DeviceModel/OpcuaDetail/${record.key}`" style="color: #13C2C2"><a-icon type="edit" />{{$t('dataModel.modelDetail')}}</router-link> |
        <router-link :to="`/DeviceModel/OpcuaNodeid/${record.key}`"  style="color: darkorange"><a-icon type="unordered-list" />{{$t('dataModel.opcuaModel.NodeIDConfig')}}</router-link> |
...
<script>
import {OpcuaModellist,OpcuaModelDelete} from "@/services/opcuaModel";
export default {
  name: 'OpcuaModelList',
  ...
  data () {
    return {
      ...
      columns: [
        ...
        {
          width: '10%',
          slotName: 'dataModel.opcuaModel.connection',
          scopedSlots: { customRender: 'OPCUAConnectType', title: 'dataModel.opcuaModel.connection' },
          dataIndex: 'OPCUAConnectType',
        },
        {
          width: '20%',
          slotName: 'dataModel.opcuaModel.Authentication',
          scopedSlots: { customRender: 'OPCUAAuthModes', title: 'dataModel.opcuaModel.Authentication' },
          dataIndex: 'OPCUAAuthModes',
        },
        {
  ...
  methods: {
    ...
    getModelList(){
      ...
      OpcuaModellist(params).then(function (res){
        let tableData={}
        ...
        if(res.data.list!=null)
        {
          for(let i=0;i<res.data.list.length;i++)
          {
            ...
            tableData.OPCUAConnectType = res.data.list[i].OPCUAConnectType
            tableData.OPCUAAuthModes = res.data.list[i].OPCUAAuthModes
    ...
    deleteRecord(key) {
      ...
      let _t = this
      OpcuaModelDelete(params).then(function (res) {
    ...
    opcuaAdd(){
      this.$router.push('/DeviceModel/OpcuaAdd')
    },
  }
}
</script>
