  ...
  <a-card :body-style="{padding: '24px 32px'}" :bordered="false">
    <a-form @submit="onSubmit" :form="form">
      ...
      <div v-if="modbusConnectType=='Serial'">
        <a-form-item
            :label="$t('dataModel.modbusModel.ModbusType')"
            :labelCol="{span: 7}"
            :wrapperCol="{span: 10}"
        >

          <a-select
              v-decorator="['ModbusType', {rules: [{ required: true, message: $t('dataModel.modbusModel.ModbusType'), whitespace: true}]}]"
          >
            <a-select-option v-for="options in ModbusSerialMode" :key="options" :value="options">
              {{ options }}
            </a-select-option>
      ...
      <div v-else-if="modbusConnectType=='TCPClient'">
        <a-form-item
            :label="$t('dataModel.modbusModel.ModbusType')"
            :labelCol="{span: 7}"
            :wrapperCol="{span: 10}"
        >
          ...
          <a-select
              v-decorator="['ModbusType', {rules: [{ required: true, message: $t('dataModel.modbusModel.ModbusType'), whitespace: true}]}]"
          >
            <a-select-option v-for="options in ModbusTCPMode" :key="options" :value="options">
              {{ options }}
            </a-select-option>
      ...
      <div v-else-if="modbusConnectType=='TCPServer'">
        <a-form-item
            :label="$t('dataModel.modbusModel.ModbusType')"
            :labelCol="{span: 7}"
            :wrapperCol="{span: 10}"
        >
          ...
          <a-select
              v-decorator="['ModbusType', {rules: [{ required: true, message: $t('dataModel.modbusModel.ModbusType'), whitespace: true}]}]"
          >
            <template v-for="options in ModbusTCPMode">
              <a-select-option  :key="options" :value="options" v-if="options!='TCP/IP'">
                {{ options }}
              </a-select-option>
            </template>
      ...
      this.form.validateFields((err) => {
        if (!err) {
          ...
          const params = {
            ...
            modbusClientIpaddress:this.form.getFieldValue('IpAddress'),
            modbusConnectType:this.form.getFieldValue('modbusConnectType'),
            modbusConnectMode:this.form.getFieldValue('ModbusType'),
            modbusCom:this.form.getFieldValue('SerialPort'),
            serialBaud:parseInt(this.form.getFieldValue('SerialPortBaud')),
            serialBits:parseInt(this.form.getFieldValue('SerialPortDataBit')),
            serialParity:this.form.getFieldValue('SerialPortVerifyBit'),
            serialStopBits:this.form.getFieldValue('SerialPortStopBit'),
            serialFlow:this.form.getFieldValue('SerialPortFlow'),
          };
          ModbusModelAdd(params).then(function (res){
            _t.messageShowLoad = false
            if (res.data.code == 2002) {
              _t.$message.success(_t.$t('dataModel.modelAddSuccess'), 3)
              _t.$router.push('/DeviceModel/ModbusModel')
            }
          }).catch(function (){
            _t.messageShowLoad = false
