...
<!--    寄存器组的寄存器地址配置-->
    <a-card  v-if="!registerGroupListTable" style="min-height: 400px">
      <a-space class="operator">
        <a-button type="primary"  @click="addRegister"> <a-icon type="folder-add"  />
          {{$t('dataModel.modbusModel.add')}}</a-button>
        <a-divider type="vertical" />
        <a-button type="primary" @click="handleExport"> <a-icon type="export" />{{$t('dataModel.export')}}</a-button>
        <a-divider type="vertical" />
        <a-upload
            name="file"
            :multiple="false"
      ...
      <div>
      <a-spin style="z-index:9999;margin-top: -600px" size="large" :spinning="ShowRegisterLoading" tip="Loading...">
        <a-drawer
          :title="isEditRegisterTrue?$t('dataModel.EditDataModel'):$t('dataModel.modbusModel.add')"
          :width="720"
          :visible="editVisible"
      ...
      >
        <a-form :form="EditForm" layout="vertical" >
          <a-row :gutter="16">
            ...
            <a-col :span="6">
              <a-form-item :label="$t('dataModel.modbusModel.RegisterAddr')">
                <a-input
                    v-decorator="[
                  'registerAddress',
                  {
                    rules: [{ required: true, message: $t('dataModel.modbusModel.RegisterAddr') }],
                  },
                ]"
                />
              </a-form-item>
            ...
            <a-col :span="8">
              <a-form-item :label="$t('dataModel.editData.dataType')">
                <a-select   autocomplete="autocomplete"  @change="changeDataType"  v-decorator="[
                  ...
                  <a-select-option value="Long64">Long64</a-select-option>
          ...
          <a-row :gutter="16">
            <a-col :span="12" v-if="dataType=='Long'||dataType=='Float'">
              ...
              <a-form-item :label="$t('dataModel.editData.floatCount')">

                <a-input   autocomplete="autocomplete"   v-decorator="[
                  'FloatAccuracy',
                  {
                    rules: [{ required: false, message: $t('dataModel.editData.floatCount') }],
                     initialValue: '2'
                  },
                ]">
                </a-input>
			    ...
			    <a-col :span="12" v-if="dataType=='Long64'||dataType=='Float64'">
              <a-form-item :label="$t('dataModel.editData.DataOrder')">
                <a-select   autocomplete="autocomplete"  v-decorator="[
                  'ByteOrder',
                  {
            ...
            <div v-else>
              ...
              <a-col :span="12" v-if="recordStatus" >
                <a-form-item :label="$t('dataModel.dataRecordType')">
                  ...
                  'dataRecordType',
                  {
                    ...
                    <a-select-option value=4>{{$t('dataModel.dataRecordExactly')}}</a-select-option>
                  </a-select>
                </a-form-item>
              </a-col>
              ...
              <a-col :span="12" v-if="(recordStatus)&&(DataRecordType==4)">
                <a-form-item :label="$t('dataModel.dataRecordExactlyInterval')">
                  <a-select   autocomplete="autocomplete"  v-decorator="[
                  'dataRecordExactlyInterval',
                  {
                    rules: [{ required: true, message: $t('dataModel.dataRecordExactlyInterval') }],
                    initialValue: '5'
                  },
                ]">
                    <a-select-option value=1>{{$t('dataModel.dataRecordExactly5')}}</a-select-option>
                    <a-select-option value=2>{{$t('dataModel.dataRecordExactly10')}}</a-select-option>
                    <a-select-option value=3>{{$t('dataModel.dataRecordExactly15')}}</a-select-option>
                    <a-select-option value=4>{{$t('dataModel.dataRecordExactly30')}}</a-select-option>
                    <a-select-option value=5>{{$t('dataModel.dataRecordExactly60')}}</a-select-option>
                  </a-select>
                </a-form-item>
              </a-col>
            </div>
        ...
        >
          <a-button  type="primary" :style="{ marginRight: '8px' }" @click="save()">
            {{isEditRegisterTrue?$t('device.EditButton'):$t('dataModel.modbusModel.add')}}
          </a-button>
...
<!--    寄存器组表格-->
    <a-card  v-if="registerGroupListTable" style="min-height: 400px">
      <a-space class="operator">
        ...
        <a-button type="primary" @click="RegisterVisible=true,isEditTrue=false"> <a-icon type="plus" />
      ...
      <a-spin style="padding: 1px;"  :spinning="messageShowLoad" tip="Loading...">
        <a-table  :pagination="pagination" row-key="uuid" :columns="registerGroupColumns" :data-source="registerGroupDataSource" class="ant-table-tbody">
          ...
          <template slot="action" slot-scope="text, record">
            <div class="editable-row-operations">
               <span >
                <a  @click="() => editRegisterConfig(record)">
                  <a-icon type="edit" /> {{$t('dataModel.modbusModel.edit')}}</a>
              </span>
              <a-divider type="vertical" />
              <span >
...
<!--    添加寄存器组-->
    <a-modal  size="large" :bodyStyle="{ width: '800px',height: '210px'}"  v-model="RegisterVisible" :title="isEditTrue?$t('dataModel.modbusModel.EditRegisterGroup'):$t('dataModel.modbusModel.RegisterGroup')" on-ok="handleOk">
...
import {
  ...
  modbusModelGroupDel, modbusModelGroupEdit,
...
export default {
  ...
  methods: {
    ...
    addRegister(){
      this.isEditRegisterTrue=false
      this.editVisible = true;
    },
    edit(item) {
      let _t = this
      this.isEditRegisterTrue=true
      ...
      setTimeout(function (){
        _t.EditForm.setFieldsValue(
            {
              name:item.name,
              registerAddress:item.registerAddress.toString(),
              dataType:item.type,
              dataAuth:item.auth,
              dataUnit:item.unit,
              NodeIDDec:item.Description,
              FloatAccuracy:item.FloatAccuracy,
              ConversionExpression:item.conversionExpression,
            })
            if(item.type=="Long"||item.type=="Float"||item.type=="Float64"||item.type=="Long64")
            {
              _t.EditForm.setFieldsValue(
                  {
              ...
              _t.EditForm.setFieldsValue(
                  {
                    ...
                    dataRecordExactlyInterval:item.RecordDataTimely.toString(),
        ...
        this.EditForm.validateFields((err) => {
          if (!err) {
            let params = {
              ...
              registerAddress:parseInt(this.EditForm.getFieldValue('registerAddress')),
              ...
              FloatAccuracy:this.EditForm.getFieldValue('FloatAccuracy'),
              ...
              RecordDataTimely:this.EditForm.getFieldValue('dataRecordExactlyInterval')?this.EditForm.getFieldValue('dataRecordExactlyInterval').toString():"",
            }
            let _t = this
            modbusModelRegisterEdit(params).then(function (res){
              if(res.data.code==200)
              {
                ...
                if (target) {
                  ...
                  target.registerAddress=_t.EditForm.getFieldValue('registerAddress')
                  ...
                  target.FloatAccuracy=_t.EditForm.getFieldValue('FloatAccuracy')
                  ...
                  target.RecordDataTimely=_t.EditForm.getFieldValue('dataRecordExactlyInterval')?_t.EditForm.getFieldValue('dataRecordExactlyInterval').toString():""
                  _t.dataSource = newData;
                }
                _t.$message.success(_t.$t("dataModel.saveSuccess"));
                _t.editVisible = false;
      ...
      else
      {
        ...
        this.EditForm.validateFields((err) => {
          if (!err) {
            ...
            modbusModelRegisterAdd(params).then(function (res){
              if(res.data.code==200)
              {
                _t.registerAddressList(_t.registerUUID)
                _t.$message.success(_t.$t("dataModel.saveSuccess"));
                _t.editVisible = false;
              }
              else
      ...
      if(this.isEditTrue)
      {
        this.RegisterForm.validateFields((err) => {
          if (!err) {
            ...
            let _t = this
            this.messageShowLoad = true
            modbusModelGroupEdit(params).then(function (res){
    ...
    editRegisterConfig(item){
      this.isEditTrue=true
      this.registerUUID = item.uuid
      this.RegisterVisible=true
      let _t = this
      setTimeout(function (){
        _t.RegisterForm.setFieldsValue(
            {
              RegisterGroupName:item.name,
              RegisterFunction:item.function.toString(),
              RegisterStartAddress:item.registerStart.toString(),
              RegisterCount:item.registerCount.toString()
            })
      },100)
    },
