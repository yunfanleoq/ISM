  ...
  <a-card>
    ...
    <a-spin style="padding: 1px;"  :spinning="messageShowLoad" tip="Loading...">
      ...
      <div slot="action" slot-scope="text, record">
        <router-link :to="`/DeviceModel/DLT645ModelDetail/${record.key}`" style="color: #13C2C2"><a-icon type="edit" />{{$t('dataModel.modelDetail')}}</router-link> |
        <router-link :to="`/DeviceModel/DLT645ModelData/${record.key}`"  style="color: darkorange"><a-icon type="import" />{{$t('dataModel.DLT645Model.DataList')}}</router-link> |
...
export default {
  ...
  methods: {
      ...
      DeviceModellist(params).then(function (res){
        ...
        if(res.data.list!=null)
        {
          for(let i=0;i<res.data.list.length;i++)
          {
            ...
            tableData.modbusConnectType = res.data.list[i].DLT645ConnectType
            tableData.modbusConnectMode = res.data.list[i].DLT645ConnectMode
    ...
    modbusAdd(){
      this.$router.push('/DeviceModel/DLT645ModelAdd')
    },
  }
}
</script>
