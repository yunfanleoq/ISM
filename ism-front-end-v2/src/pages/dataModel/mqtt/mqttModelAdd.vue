...
<script>
import {MqttModelAdd, MqttModelDelete,MqttModelEdit,MqttModelList} from "@/services/mqttModel";
    ...
    onSubmit (e) {
      ...
      this.form.validateFields((err) => {
        if (!err) {
          ...
          MqttModelAdd(params).then(function (res){
            _t.messageShowLoad = false
            if (res.data.code == 2002) {
              _t.$message.success(_t.$t('dataModel.modelAddSuccess'), 3)
              _t.$router.push('/DeviceModel/MqttModel')
            }
          }).catch(function (){
            _t.messageShowLoad = false
