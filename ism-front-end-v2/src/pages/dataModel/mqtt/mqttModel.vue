<template>
  <a-card>
      <a-space class="operator">
        <a-button @click="MqttAdd()" type="primary" icon="plus">{{$t('dataModel.newModel')}}</a-button>
    ...
    <a-spin style="padding: 1px;"  :spinning="messageShowLoad" tip="Loading...">
      ...
      <div slot="action" slot-scope="text, record">
        <router-link :to="`/DeviceModel/MqttDetail/${record.key}`" style="color: #13C2C2"><a-icon type="edit" />{{$t('dataModel.modelDetail')}}</router-link> |
        <router-link :to="`/DeviceModel/MqttNodeid/${record.key}`"  style="color: darkorange"><a-icon type="unordered-list" />{{$t('dataModel.opcuaModel.NodeIDConfig')}}</router-link> |
...
<script>
import {MqttModelList,MqttModelDelete} from "@/services/mqttModel";
export default {
  name: 'MQTTModelList',
  ...
  methods: {
    ...
    getModelList(){
      ...
      MqttModelList(params).then(function (res){
        let tableData={}
    ...
    deleteRecord(key) {
      ...
      let _t = this
      MqttModelDelete(params).then(function (res) {
    ...
    MqttAdd(){
      this.$router.push('/DeviceModel/MqttAdd')
    },
  }
}
</script>
