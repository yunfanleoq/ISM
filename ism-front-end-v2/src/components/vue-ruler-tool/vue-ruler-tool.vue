...
export default {
  ...
  computed: {
    wrapperStyle() {
      let windowWidthString = (this.windowWidth+200)
      let windowHeightString = (this.windowHeight+200)
      return {
        width : windowWidthString.toString() + 'px',
        height :windowHeightString.toString() + 'px',
  ...
  methods: {
    ...
    rightGuide(e,id){
      ...
      this.$refs.guideMenu.showContextMenu(e.pageX,e.pageY)
    },
    deleteGuideLine(){
      this.lineList.splice(0,1)
      this.$refs.guideMenu.hide()
    },
    windowResize() {
      this.xScale = []
      this.yScale = []
    ...
    box () {
      ...
      // } else {
      //   this.windowWidth = document.body.clientWidth - this.leftSpacing
      //   this.windowHeight = document.body.clientHeight - this.topSpacing+150
      // }
      this.rulerWidth = this.windowWidth+200
      this.rulerHeight = this.windowHeight+200
      this.setSpacing()
    }, // 获取窗口宽与高
    setSpacing () {
      ...
      if (this.isHotKey) {
        switch ($event.keyCode) {
          ...
          case this.keyCode.h:
            ...
            this.$emit('update:guide', this.guide)
            break
        }
      }
    }, // 键盘事件
