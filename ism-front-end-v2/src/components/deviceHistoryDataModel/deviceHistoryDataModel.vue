<template>
  <div ref="dataModel">
    <a-modal :visible="dataModelDialog"
        ...
        <div style="margin: 6px">
            <a-card size="small" :title="$t('component.deviceDataModel.DataFrom')" region="north" >
              <a-form layout="inline" :label-col="{ span: 5}" :wrapper-col="{ span: 19 }">
              <a-form-item :label="$t('component.deviceDataModel.selectDeviceType')">
                <a-select @change="changeDeviceType" style="width:400px" :dropdownStyle="{'z-index': 9999999}">
        ...
        <a-spin :spinning="messageShowLoad" style="min-height: 300px">
          <div style="margin: 6px">
            <a-layout >
              <a-layout-sider style="background: #fff;" :bodyStyle="{padding:'2px'}" :panelStyle="{width:'200px',minWidth:'100px'}" :title="$t('component.deviceDataModel.DataModelList')" v-if="(fromType==1)&&(DeviceType!=6&&DeviceType!=7)" region="west" :split="false" >
<!--                <Tree :data="modelList"  @selectionChange="changeDeviceModel($event)"></Tree>-->
                <a-card size="small" :hoverable="true" :title="$t('component.deviceDataModel.DataModelList')" style="min-height: 340px">
                  <a-directory-tree
                      :tree-data="modelList"
                      :replace-fields="{ key: 'uuid',title:'text'}"
                      @select="changeDeviceModel">
                  </a-directory-tree>
                </a-card>
              </a-layout-sider>
              <a-layout-content style="background: #fff;" :title="$t('component.deviceDataModel.DataList')" :split="false" :bodyStyle="{padding:'2px','min-height':'300px'}" :panelStyle="{width:((fromType==1)&&(DeviceType!=6&&DeviceType!=7))?'500px':'700px',height:'100%',minWidth:'100px'}">
                <a-table
                    row-key="uuid"
                    :customRow="rowClick"
                    :columns="columns"
...
import {getDatasByUuid, getHistoryDatasByUuid, snmpModelList} from "@/services/snmpmodel";
import {getMonitorTree} from "@/services/device";
import {GetDataModelData, GetSystemData} from "@/services/system";
export default {
  name: 'deviceHistoryDataModel',
  ...
  activated(){
    ...
    this.changeDeviceType(this.DeviceType)
  },
  mounted(){
    console.log("mounted")
    if(this.fromType==2)
  ...
  methods: {
    ...
    getDeviceHistoryDataModel(uuid){
      let params={
        muid:uuid,
        type:this.DeviceType
      }
      ...
      let _t = this
      this.messageShowLoad = true
      getHistoryDatasByUuid(params).then(function (res){
        _t.messageShowLoad = false
        if((res.data.mibs!=null)&&(res.data.mibs.length>0))
        {
          let temp={}
    ...
    getSupportDevice(){
      ...
      getSupportDeviceList().then(function (res){
        ...
        _t.supportDeviceList.push({"name":"component.deviceDataModel.SystemVar",type:6})
        _t.supportDeviceList.push({"name":"device.CustomDevice",type:7})
      })
    },
    selectData(ev){

    },
    SelectDevice(ev,node,rt){
