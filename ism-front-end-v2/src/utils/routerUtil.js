...
function loadRoutes(routesConfig) {
  ...
  if (asyncRoutes) {
    if (routesConfig && routesConfig.length > 0) {
      ...
      const merged = mergeRoutes(basicOptions.routes, routes)
      const finalRoutes = dedupRoutesByName(merged)
...
/**
 * 递归去重路由（按 name），避免同名路由被重复注册
 */
function dedupRoutesByName(routes, seen) {
