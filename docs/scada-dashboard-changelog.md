...
## 坑点：AppRun 整屏白屏 = 首页 cells 被清空

**现象**：`#/AppRun/{MODEL_ID}` 打开后整屏白，左侧树/KPI/拓扑全无；前端能编译、能登录、其它明细页都正常。

**根因**：`display_model_layer` 中**首页行**（`is_home=1`、`page_name='main'`、`page_id=MODEL_ID`）
的 `components` 被写成空数组 —— base64 解出来是 `{"cells":[]}`（仅 16 字节），
而 building/floor/device 明细页都是满的（17万~26万字节）。前端首页无 cell 可渲染，故白屏。
（与前端 `comp.default.data` 容错回归是两码事；那个已修，这个是**数据层**问题。）

**一分钟定位**（OceanBase：`root@ism_tenant`/`ism2024!`@127.0.0.1:2881，库 `ism`）：
```python
# 查首页 cells 长度；≈16 即空 → 白屏
select is_home, page_name, length(components) from display_model_layer
where model_id='043135ad-44be-e5d8-89be-3e54883c23a8' and is_home=1;
```

**修复（一条命令）**：
```bash
python3 build_ncc_dashboard.py   # 重新生成总览(104 cells)并 UPDATE 写回首页行
```
> 跑完应看到 "Level 0 (overview): 104 cells"，首页 `length(components)` 由 16 → ~170832。
> 验证：`http://localhost:7080/#/AppRun/043135ad-44be-e5d8-89be-3e54883c23a8` 出现 76/76 在线、左侧树、拓扑、趋势。

## 坑点：左侧树点击钻探 = 右侧白屏（page not found）

**现象**：左侧导航树能展开/收起，但点任意节点右侧整屏白；`currentPageUUID` 被清空。

**根因**：`ISMRunTreeNav.vue` 的 `transform()` 用 `v.Sid`（大写）取机柜/设备 sid，但
`monitortree` 接口返回的节点 `value` 字段是**小写 `sid`** —— `v.Sid` 恒为 `undefined`，
导致所有 page_id 退化成 `uuid5('ncc-dash-bldg/floor/dev-undefined')`，与 `build_ncc_dashboard.py`
写入的 page_id 全部错位。前端取页是在 Vuex `ISMDisPlayEditorTool.PCPageList` 里按 `pageUuid`
精确匹配，匹配不到就 `isFound=false`→`showPage` 走 "page not found" 分支并清空画布 → 白屏。

**定位要点**：
- `currentPageUUID===""` + 画布 HTML≈空 = 命中 `isFound===false` 分支（ISMRender `showPage`）。
- 直接 `$EventBus.$emit('GoPage',{ModelId, PageUuid:<库里真实page_id>})` 能正常渲染 → 证明
  showPage/渲染没问题，是树算的 page_id 不对。
- 反查：`uuid5('ncc-dash-bldg-undefined').hex == 树算出的值` → 实锤 sid 取成了 undefined。
- monitortree 节点 `value` 真实字段：小写 `sid`/`type`/`muid`/`name`/`uuid`，大写 `Status`。

**修复**：`ISMRunTreeNav.vue` `transform()` 中 `const sid = v.Sid` → `const sid = v.sid`
...
- **2026-06-17**：定位并修复 AppRun 整屏白屏 —— 首页 `is_home=1` 行 `components` 为空 `{"cells":[]}`，
  ...
  另确认此前的"启动卡 68%"是误用双重后台（`nohup ... &` 叠加工具后台）导致进程被回收，**非内存问题**；
  正确启动用工具自身后台（不带内层 `&`），20G 内存约 20s 即 "Compiled successfully"。
